...
import JefeAgentesPanel from "./components/JefeAgentesPanel";
import ResumenJefesVentas from "./components/ResumenJefesVentas";
import SearchableSelect from "./components/SearchableSelect";
import PendingAlertModal from "./components/PendingAlertModal";
import { OperationRecord, AppSettings, UserRole, UserAccount } from "./types";
import * as api from "./services/api";
import { normalizeText, formatProjectAndUnits, formatUnitDisplay, checkDuplicateUnitOperation, isOtherAreasObservation } from "./utils/dateUtils";
...
export default function App() {
  ...
  // Layout States
  const [activeTab, setActiveTab] = useState<"flow" | "spreadsheet" | "kpis" | "ventasReport">("flow");
  const [loading, setLoading] = useState(true);
  const [errorMsg, setErrorMsg] = useState<string | null>(null);
  const [refreshing, setRefreshing] = useState(false);
  const [rectifyingToday, setRectifyingToday] = useState(false);
  const [rectifyResultMsg, setRectifyResultMsg] = useState<string | null>(null);

  const handleRectifyTodayRecords = async () => {
    try {
      setRectifyingToday(true);
      const res = await api.rectifyTodayRecords(true);
      await fetchDb(true);
      if (res.count > 0) {
        setRectifyResultMsg(`Se rectificaron ${res.count} registros de hoy correctamente.`);
      } else {
        setRectifyResultMsg("Todos los registros de hoy ya se encuentran correctamente clasificados.");
      }
      setTimeout(() => setRectifyResultMsg(null), 5000);
    } catch (e: any) {
      setRectifyResultMsg("Error al rectificar: " + (e?.message || e));
      setTimeout(() => setRectifyResultMsg(null), 5000);
    } finally {
      setRectifyingToday(false);
    }
  };

  // Fetch Database (Records + Settings) from Server
  const fetchDb = async (forceRefresh = false) => {
    try {
      setRefreshing(true);
      const { records: recordsData, settings: settingsData } = await api.getBootstrapData(forceRefresh);
  ...
  useEffect(() => {
    // 1. Initial full fetch + auto-rectify records recorded today
    fetchDb().then(() => {
      api.rectifyTodayRecords(true).then(res => {
        if (res.count > 0) {
          console.log(`[Auto-Rectify] Rectificados ${res.count} registros de hoy.`);
        }
      }).catch(err => {
        console.warn("[Auto-Rectify] Notice:", err);
      });
    });

    // 2. Real-time operations subscription (across all connected clients)
    const unsubscribeRecords = api.subscribeToRecords((liveRecords) => {
      const uniqueRecords: OperationRecord[] = [];
      const seenIds = new Set<string>();
    ...
    // 3. Real-time settings subscription
    const unsubscribeSettings = api.subscribeToSettings((liveSettings) => {
      setSettings(liveSettings);

      // Re-hydrate currentUser permissions/roles if updated
      const savedUserStr = localStorage.getItem("operaciones_session");
      if (savedUserStr) {
        try {
          const parsed = JSON.parse(savedUserStr) as UserAccount;
          const fresh = liveSettings.users?.find((u: UserAccount) => u.username === parsed.username);
          if (fresh && fresh.active !== false) {
            setCurrentUser(fresh);
            localStorage.setItem("operaciones_session", JSON.stringify(fresh));
          }
        } catch (e) {}
      }
    });
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (foundUser.username.toLowerCase() === "admin") {
      expectedPassword = foundUser.password || "1506";
      if (expectedPassword === "0000" || expectedPassword === "admin") {
        expectedPassword = "1506";
      }
    }
    ...
    if (foundUser.password === "0000" && foundUser.username.toLowerCase() !== "admin") {
  ...
  const handleAddRecord = async (recordData: Partial<OperationRecord>) => {
    try {
      ...
      // Validation: Prevent duplicate units in the same project
      const duplicateCheck = checkDuplicateUnitOperation(records, finalData);
      ...
      if (!finalData.derivadoA && finalData.proyecto) {
        const assistants = settings.users?.filter(
          u => u.role === "Asistente Legal" && u.active && u.assignedProjects?.some(p => normalizeText(p) === normalizeText(finalData.proyecto))
        ) || [];
      ...
      setRecords(prev => {
        // Guarantee no duplicates if the record was already added by onSnapshot listener
        const filtered = prev.filter(r => r.id !== added.id);
  ...
  const handleUpdateRecord = async (id: string, updatedFields: Partial<OperationRecord>) => {
    try {
      ...
      // Validation: Prevent duplicate units in the same project when units or project are updated
      if (
        finalFields.dpto !== undefined ||
        finalFields.estac !== undefined ||
        finalFields.dep !== undefined ||
        finalFields.proyecto !== undefined
      ) {
        const existingRec = records.find(r => r.id === id);
        if (existingRec) {
          const targetProj = finalFields.proyecto !== undefined ? finalFields.proyecto : existingRec.proyecto;
          const targetDpto = finalFields.dpto !== undefined ? finalFields.dpto : existingRec.dpto;
          const targetEstac = finalFields.estac !== undefined ? finalFields.estac : existingRec.estac;
          const targetDep = finalFields.dep !== undefined ? finalFields.dep : existingRec.dep;
          const targetTipo = finalFields.tipo !== undefined ? finalFields.tipo : existingRec.tipo;
      ...
      if (finalFields.proyecto && !finalFields.derivadoA) {
        const assistants = settings.users?.filter(
          u => u.role === "Asistente Legal" && u.active && u.assignedProjects?.some(p => normalizeText(p) === normalizeText(finalFields.proyecto))
        ) || [];
        if (assistants.length > 0) {
      ...
      setRecords(prev => {
        ...
        const updatedList: OperationRecord[] = [];
        let matched = false;
        for (const r of prev) {
          const item = r.id === id ? updated : r;
          if (item.id) {
            if (!seen.has(item.id)) {
              seen.add(item.id);
  ...
  const canViewKpis = () => {
    ...
    if (currentUser.role === "Administrador" || currentUser.role === "Jefe Legal" || currentUser.role === "Jefe de Agentes") {
      return true; // Always allow Admin, Jefe Legal, and Jefe de Agentes (has their dedicated KPI dashboard)
    }
    return settings.kpiVisibility?.[currentUser.role] === true;
  };

  // Filtered records for the current user's visibility scope in widgets & sidebars
  const visibleSidebarRecords = records.filter(r => {
    if (!currentUser) return false;
    
    // Asistente Legal: ONLY their assigned projects or operations specifically derived to them
    if (currentUser.role === "Asistente Legal") {
      const assigned = currentUser.assignedProjects || [];
      const normProj = normalizeText(r.proyecto);
      const isProjAssigned = assigned.some(p => normalizeText(p) === normProj);
      const isDerived = r.derivadoA && normalizeText(r.derivadoA) === normalizeText(currentUser.username);
      return isProjAssigned || isDerived;
    }

    // Jefe de Ventas: only their assigned projects (excluding Agentes team)
    if (currentUser.role === "Jefe de Ventas") {
      const userProjects = (settings.proyectos || [])
        .filter(p => p.jefeVentas === currentUser.username)
        .map(p => normalizeText(p.name));
      const normProj = normalizeText(r.proyecto);
      const isAgente = (r.team || "").toUpperCase().includes("AGENTE");
      return userProjects.includes(normProj) && !isAgente;
    }

    // Jefe de Agentes: full visibility across all projects ("podra ver todos los proyectos")
    if (currentUser.role === "Jefe de Agentes") {
      return true;
    }

    // Administrador & Jefe Legal: full access
    return true;
  });

  // Helper to determine status display and visual color
  // Fully dynamic based on settings.statusColors (Image 1 config & Admin controls)
  const getOperationStatusInfo = (r: OperationRecord) => {
    const rawStatus = (r.status || "").trim();
    const hasEmission = Boolean(r.emision && r.emision.trim()) || Boolean(r.emittedAt);
    const hasHistoryActions = Boolean(
      r.history &&
      r.history.some(h => {
        const c = (h.comentario || "").toLowerCase();
        return c && !c.includes("inicial") && !c.includes("registrada por jefe") && !c.includes("creado por jefe");
      })
    );

    const hasAnyAction = hasEmission || hasHistoryActions;
    const isExplicitPending = !rawStatus || rawStatus.toLowerCase().includes("pendiente");

    const basePill = "px-2 py-0.5 rounded-lg border text-[9px] font-bold text-center inline-block shadow-2xs whitespace-nowrap";

    // If no action has been registered yet OR status is explicitly Pendiente
    if (!hasAnyAction || isExplicitPending) {
      const customPending = settings.statusColors?.["Pendiente"] || settings.statusColors?.["Pendiente de Firma"];
      return {
        label: rawStatus || "Pendiente",
        className: customPending ? `${basePill} ${customPending}` : `${basePill} bg-rose-50 text-rose-700 border-rose-200`
      };
    }

    // Direct lookup in settings.statusColors
    if (settings.statusColors && settings.statusColors[rawStatus]) {
      return {
        label: rawStatus,
        className: `${basePill} ${settings.statusColors[rawStatus]}`
      };
    }

    // Normalized lookup in settings.statusColors
    if (settings.statusColors) {
      const matchKey = Object.keys(settings.statusColors).find(
        k => k.toLowerCase().trim() === rawStatus.toLowerCase().trim()
      );
      if (matchKey && settings.statusColors[matchKey]) {
        return {
          label: rawStatus,
          className: `${basePill} ${settings.statusColors[matchKey]}`
        };
      }
    }

    // Fallbacks matched to Image 1 palette
    const sLower = rawStatus.toLowerCase();
    if (sLower.includes("acuerdo interno")) {
      const c = settings.statusColors?.["ACUERDO INTERNO"] || "bg-purple-100 text-purple-800 border-purple-300";
      return { label: rawStatus, className: `${basePill} ${c}` };
    }
    if (sLower.includes("modificad")) {
      const c = settings.statusColors?.["Modificado"] || "bg-amber-100 text-amber-800 border-amber-300";
      return { label: rawStatus, className: `${basePill} ${c}` };
    }
    if (sLower.includes("revisión") || sLower.includes("revision")) {
      const c = settings.statusColors?.["En Revisión"] || settings.statusColors?.["En Revisión Técnica"] || "bg-blue-100 text-blue-800 border-blue-300";
      return { label: rawStatus, className: `${basePill} ${c}` };
    }
    if (sLower.includes("emitid") || sLower.includes("aprobado")) {
      const c = settings.statusColors?.["Emitido"] || settings.statusColors?.["Aprobado para Emisión"] || "bg-emerald-100 text-emerald-800 border-emerald-300";
      return { label: rawStatus, className: `${basePill} ${c}` };
    }
    if (sLower.includes("observad") || sLower.includes("rechazad")) {
      const c = settings.statusColors?.["Observado / Rechazado"] || "bg-rose-100 text-rose-800 border-rose-300";
      return { label: rawStatus, className: `${basePill} ${c}` };
    }
    if (sLower.includes("cierre completo")) {
      const c = settings.statusColors?.["Cierre Completo"] || "bg-blue-50 text-blue-700 border-blue-300";
      return { label: rawStatus, className: `${basePill} ${c}` };
    }
    if (sLower.includes("otras áreas") || sLower.includes("otras areas") || isOtherAreasObservation(r)) {
      const c = settings.statusColors?.["Observaciones de otras áreas"] || "bg-blue-50 text-blue-700 border-blue-300";
      return { label: rawStatus, className: `${basePill} ${c}` };
    }
    if (sLower.includes("desistid")) {
      const c = settings.statusColors?.["Desistido"] || "bg-slate-100 text-slate-800 border-slate-200";
      return { label: rawStatus, className: `${basePill} ${c}` };
    }

    return {
      label: rawStatus,
      className: `${basePill} bg-blue-50 text-blue-700 border-blue-200`
    };
  };
  ...
  // LOGIN SCREEN (Strictly matching Image 1 & Image 2)
  if (!currentUser) {
    // Only enabled active users as shown in Image 2
    const desiredOrder = ["admin", "marilyn saona", "sthief vidalon", "daniel hurtado", "franco odar"];
    const activeUsers = (settings.users || []).filter(u => u.active !== false && u.active === true);
    activeUsers.sort((a, b) => {
      const idxA = desiredOrder.indexOf(a.username.toLowerCase());
      const idxB = desiredOrder.indexOf(b.username.toLowerCase());
      ...
      <div className="min-h-screen bg-blue-50/50 flex flex-col justify-center items-center p-4 antialiased">
        ...
        <div className="mb-6 text-center space-y-2">
          {settings.platformLogo ? (
            ...
            <div className="bg-brand-primary text-white p-3.5 rounded-2xl shadow-xl shadow-blue-200 inline-flex items-center justify-center">
              <span className="material-symbols-outlined text-[32px] font-bold block">grid_view</span>
            </div>
          )}
          <div>
            <h1 className="font-extrabold text-xl text-brand-primary tracking-tight uppercase">
              EMISIONES DE MINUTAS - EQUIPO LEGAL
            </h1>
            <p className="text-[11px] text-slate-500 font-medium">
    ...
    <div className="min-h-screen bg-blue-50/50 font-sans antialiased text-slate-800 flex flex-col">
      ...
      <header className="bg-white border-b border-blue-100 sticky top-0 z-40 shadow-xs px-4 md:px-8 py-3 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 self-end md:self-auto">
          ...
          <div className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 bg-emerald-50 border border-emerald-200/80 rounded-xl text-[10px] font-bold text-emerald-800 shadow-xs" title="Sincronización bidireccional en tiempo real activa para todas las computadoras conectadas">
          ...
          {/* Rectify today records button */}
          {(currentUser.role === "Administrador" || currentUser.role === "Jefe Legal") && (
            <button
              onClick={handleRectifyTodayRecords}
              disabled={rectifyingToday}
              title="Rectificar registros del día: sincroniza banderas de afectación y estados de acuerdo a las observaciones marcadas"
              className="flex items-center gap-1.5 px-2.5 py-1.5 bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-200 rounded-xl text-[10px] font-bold cursor-pointer transition-colors shadow-2xs"
            >
              <RotateCcw className={`h-3.5 w-3.5 text-amber-600 ${rectifyingToday ? "animate-spin" : ""}`} />
              <span className="hidden lg:inline">{rectifyingToday ? "Rectificando..." : "Rectificar Hoy"}</span>
            </button>
          )}
          ...
          <button
            ...
            title="Sincronizar y actualizar base de datos"
      ...
      {/* Toast message for rectification */}
      {rectifyResultMsg && (
        <div className="bg-emerald-50 border-b border-emerald-200 text-emerald-900 px-4 py-2.5 text-xs font-bold flex items-center justify-center gap-2 shadow-xs animate-fadeIn">
          <CheckCircle2 className="h-4 w-4 text-emerald-600 shrink-0" />
          <span>{rectifyResultMsg}</span>
        </div>
      )}
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 md:p-8 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          {/* Main Column: Active Views & Forms */}
          <div className={`${activeTab === "kpis" || activeTab === "ventasReport" ? "lg:col-span-12" : "lg:col-span-7"} space-y-6`}>
            
            {/* Unified tab system depending on permissions */}
            <div className="bg-white p-1 rounded-xl border border-blue-100 flex flex-wrap items-center gap-1 shadow-sm max-w-2xl">
              <button
                onClick={() => setActiveTab("flow")}
                className={`flex-1 min-w-28 flex items-center justify-center gap-1.5 py-1.5 px-3 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                  activeTab === "flow"
              ...
              {currentUser.role !== "Jefe de Ventas" && currentUser.role !== "Asistente Legal" && (
                <button
                  ...
                  className={`flex-1 min-w-20 flex items-center justify-center gap-1.5 py-1.5 px-3 rounded-lg text-xs font-bold transition-all cursor-pointer ${
              ...
              {canViewKpis() && (
                <button
                  ...
                  className={`flex-1 min-w-32 flex items-center justify-center gap-1.5 py-1.5 px-3 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                    activeTab === "kpis"
                      ? "bg-brand-primary text-white shadow-xs"
                      : "text-slate-600 hover:bg-slate-50"
                  }`}
              ...
              {/* Reporte Jefes de Ventas tab - Oculto para Asistentes Legales */}
              {currentUser.role !== "Asistente Legal" && (
                <button
                  onClick={() => setActiveTab("ventasReport")}
                  className={`flex-1 min-w-36 flex items-center justify-center gap-1.5 py-1.5 px-3 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                    activeTab === "ventasReport"
                      ? "bg-brand-primary text-white shadow-xs"
                      : "text-slate-600 hover:bg-slate-50"
                  }`}
                >
            ...
            <div className="transition-all">
              ...
              {activeTab === "ventasReport" && currentUser.role !== "Asistente Legal" && (
                <ResumenJefesVentas
                  records={records}
                  settings={settings}
                  currentUser={currentUser}
          ...
          {activeTab !== "kpis" && activeTab !== "ventasReport" && (
            ...
            <div className="bg-white p-5 rounded-2xl border border-blue-100 shadow-sm flex flex-col justify-between">
                ...
                <div className="overflow-x-auto border border-blue-50 rounded-xl">
                  <table className="w-full text-left border-collapse text-[10px]">
                    ...
                    <tbody className="divide-y divide-blue-50">
                      {visibleSidebarRecords.slice(0, 6).map(r => {
                        const statusInfo = getOperationStatusInfo(r);
                        const unitDisplay = formatUnitDisplay(r);
                        return (
                          <tr key={r.id} className="hover:bg-blue-50/10">
                            <td className="p-2 font-bold text-slate-700">{r.proyecto || "-"}</td>
                            <td className="p-2">
                              {unitDisplay && unitDisplay !== "Sin unidad" ? (
                                <span className="inline-block bg-blue-50/90 text-brand-primary border border-blue-150 px-2 py-0.5 rounded-md font-mono font-bold text-[9px]">
                                  {unitDisplay}
                                </span>
                              ) : (
                                <span className="text-slate-400 font-mono text-[9px] italic">-</span>
                              )}
                            </td>
                      ...
                      {visibleSidebarRecords.length === 0 && (
                        <tr>
                          <td colSpan={5} className="p-4 text-center text-slate-400 italic">No hay filas en la base de datos para tus proyectos.</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
              ...
              <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-500">
                <span className="flex items-center gap-1 font-semibold text-slate-600">
                  ...
                  Total Operaciones: {visibleSidebarRecords.length}
                </span>
                {settings.sharedExcelLink && currentUser.role !== "Jefe de Ventas" && currentUser.role !== "Asistente Legal" && (
                  <a
                    href={settings.sharedExcelLink}
      ...
      {/* Warning alerts for pending operations (>6, 12, 18, 24 hours) */}
      <PendingAlertModal records={records} currentUser={currentUser} />
    </div>
  );
}
