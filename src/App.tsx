...
export default function App() {
  ...
  // Requirement: If no action has been done yet, it must display as "Pendiente" in red.
  const getOperationStatusInfo = (r: OperationRecord) => {
    ...
    if (!hasAnyAction || isExplicitPending) {
      return {
        label: "Pendiente",
        className: "bg-red-50 text-red-700 text-[9px] font-extrabold px-1.5 py-0.5 rounded border border-red-200 shadow-2xs inline-flex items-center gap-1"
      };
    }
    ...
    if (sLower.includes("otras áreas") || sLower.includes("otras areas") || isOtherAreasObservation(r)) {
      return {
        label: rawStatus || "Observaciones de otras áreas",
        className: "bg-blue-50 text-blue-700 text-[9px] font-bold px-1.5 py-0.5 rounded border border-blue-300 shadow-2xs"
      };
    }

    if (sLower.includes("cierre completo") || sLower.includes("aprobado")) {
      return {
        label: rawStatus,
    ...
    if (sLower.includes("observad") || sLower.includes("rechazad")) {
      return {
        label: rawStatus,
        className: "bg-amber-50 text-amber-800 text-[9px] font-bold px-1.5 py-0.5 rounded border border-amber-200"
      };
    }
    ...
    if (sLower.includes("desistid")) {
      return {
        label: rawStatus,
        className: "bg-slate-100 text-slate-600 text-[9px] font-bold px-1.5 py-0.5 rounded border border-slate-200"
      };
    }

    return {
      label: rawStatus,
      className: "bg-blue-50 text-blue-700 text-[9px] font-bold px-1.5 py-0.5 rounded border border-blue-100"
    };
  };
