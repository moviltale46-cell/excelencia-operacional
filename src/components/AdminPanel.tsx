...
import { 
  ...
  ObservationReasonConfig, ObservationCategory, STANDARD_OBSERVATIONS, DEFAULT_OBSERVATION_REASONS
} from "../types";
import { 
  Settings, Check, Save, Copy, FileSpreadsheet, Eye, EyeOff, 
  Edit, Trash2, ShieldAlert, UserCheck, Plus, X, List, Shield, ShieldCheck,
  Building, CheckSquare, Square, Layers, HelpCircle, Info,
  RefreshCw, Zap, Server, Database, Activity, CheckCircle2, AlertCircle,
  Calendar, Clock, BarChart3, TrendingUp, Building2, User, MessageSquare,
  History, Search, RotateCcw, Wrench
} from "lucide-react";
import SearchableSelect from "./SearchableSelect";
import StatusHistoryModal from "./StatusHistoryModal";
import { clearAllCache, rectifyTodayRecords } from "../services/api";
...
export const STATUS_COLOR_PRESETS = [
  { name: "Ámbar / Amarillo (Modificado)", bg: "bg-amber-100 text-amber-800 border-amber-300" },
  { name: "Azul (En Revisión)", bg: "bg-blue-100 text-blue-800 border-blue-300" },
  { name: "Verde / Esmeralda (Emitido)", bg: "bg-emerald-100 text-emerald-800 border-emerald-300" },
  { name: "Rosa / Rojo (Observado / Rechazado)", bg: "bg-rose-100 text-rose-800 border-rose-300" },
  { name: "Púrpura / Violeta (Acuerdo Interno)", bg: "bg-purple-100 text-purple-800 border-purple-300" },
  { name: "Azul Borde (Cierre Completo)", bg: "bg-blue-50 text-blue-700 border-blue-300" },
  { name: "Rojo Suave (Pendiente)", bg: "bg-rose-50 text-rose-700 border-rose-200" },
  { name: "Pizarra / Gris (Inactivo / Desistido)", bg: "bg-slate-100 text-slate-800 border-slate-200" }
];
...
export default function AdminPanel({
  ...
  const [colorToastMsg, setColorToastMsg] = useState<string | null>(null);
  ...
  const [rectifyMsg, setRectifyMsg] = useState<{ type: "success" | "error"; text: string } | null>(null);

  const handleAdminRectify = async (onlyToday: boolean) => {
    try {
      setRectifying(true);
      setRectifyMsg(null);
      const res = await rectifyTodayRecords(onlyToday);
      if (onRefreshAll) {
        await onRefreshAll();
      }
      setRectifyMsg({
        type: "success",
        text: `Operación exitosa: ${res.count} registro(s) rectificados y sincronizados.`
      });
      setTimeout(() => setRectifyMsg(null), 6000);
    } catch (e: any) {
      setRectifyMsg({
        type: "error",
        text: "Error al rectificar: " + (e?.message || e)
      });
      setTimeout(() => setRectifyMsg(null), 6000);
    } finally {
      setRectifying(false);
  ...
  const [userFilterActive, setUserFilterActive] = useState<"all" | "active">("active");

  // Advisors management state
  const [asesores, setAsesores] = useState<string[]>(
    [...(settings.asesores || [])].sort((a, b) => a.localeCompare(b, undefined, { sensitivity: "base" }))
  );
  ...
  const [newReasonColor, setNewReasonColor] = useState<string>("#e11d48");
  const [editingReasonId, setEditingReasonId] = useState<string | null>(null);
  const [editingReasonText, setEditingReasonText] = useState("");
  const [editingReasonColor, setEditingReasonColor] = useState<string>("#e11d48");
  const [reasonsSuccessMsg, setReasonsSuccessMsg] = useState(false);
  const [newReasonCategory, setNewReasonCategory] = useState<ObservationCategory>("legal");
  const [newReasonAffectsAdvisor, setNewReasonAffectsAdvisor] = useState<boolean>(true);
  const [editingReasonCategory, setEditingReasonCategory] = useState<ObservationCategory>("legal");
  const [editingReasonAffectsAdvisor, setEditingReasonAffectsAdvisor] = useState<boolean>(true);

  // Full Master Edit Modal State (Admin - direct replacement without history trail)
  ...
  const [fullEditRecordId, setFullEditRecordId] = useState<string | null>(null);
  const [feTeam, setFeTeam] = useState("");
  const [feProyecto, setFeProyecto] = useState("");
  const [feDpto, setFeDpto] = useState("");
  const [feEstac, setFeEstac] = useState("");
  const [feDep, setFeDep] = useState("");
  const [feAsesor, setFeAsesor] = useState("");
  const [feDerivadoA, setFeDerivadoA] = useState("");
  const [feTipo, setFeTipo] = useState("");
  const [feSolicitud, setFeSolicitud] = useState("");
  const [feSolicitudAt, setFeSolicitudAt] = useState("");
  const [feEmision, setFeEmision] = useState("");
  const [feEmittedAt, setFeEmittedAt] = useState("");
  const [feCreatedAt, setFeCreatedAt] = useState("");
  const [feStatus, setFeStatus] = useState("");
  const [feComentario, setFeComentario] = useState("");
  const [feTipoObservacion, setFeTipoObservacion] = useState<"Asesor" | "Observaciones de otras áreas">("Asesor");
  const [feAffectsAdvisor, setFeAffectsAdvisor] = useState<boolean>(true);
  const [feObservationReasons, setFeObservationReasons] = useState<string[]>([]);

  // Sub-panel 5 search and filter state
  const [showAdminAuditModal, setShowAdminAuditModal] = useState(false);
  const [adminRecordSearch, setAdminRecordSearch] = useState("");
  const [adminProjectFilter, setAdminProjectFilter] = useState("TODOS");

  const filteredAdminRecords = useMemo(() => {
    let list = records;
    if (adminProjectFilter !== "TODOS") {
      list = list.filter(
        (r) => (r.proyecto || "").trim().toLowerCase() === adminProjectFilter.trim().toLowerCase()
      );
    }
    if (adminRecordSearch.trim()) {
      const q = adminRecordSearch.trim().toLowerCase();
      list = list.filter((r) => {
        const proj = (r.proyecto || "").toLowerCase();
        const dpto = (r.dpto || "").toLowerCase();
        const estac = (r.estac || "").toLowerCase();
        const dep = (r.dep || "").toLowerCase();
        const asesor = (r.asesor || "").toLowerCase();
        const status = (r.status || "").toLowerCase();
        const team = (r.team || "").toLowerCase();
        const comentario = (r.comentario || "").toLowerCase();
        const id = (r.id || "").toLowerCase();
        const combined = `${proj} ${dpto} ${estac} ${dep}`.toLowerCase();
        return (
          proj.includes(q) ||
          dpto.includes(q) ||
          estac.includes(q) ||
    ...
    return list;
  }, [records, adminProjectFilter, adminRecordSearch]);
  ...
  React.useEffect(() => {
    if (settings) {
      ...
      if (settings.proyectos) {
        setProyectos([...settings.proyectos].sort((a, b) => a.name.localeCompare(b.name, undefined, { sensitivity: "base" })));
      }
      if (settings.asesores) {
        setAsesores([...settings.asesores].sort((a, b) => a.localeCompare(b, undefined, { sensitivity: "base" })));
      }
  ...
  const [proyectos, setProyectos] = useState<ProjectConfig[]>(
    [...(settings.proyectos || [])].sort((a, b) => a.name.localeCompare(b.name, undefined, { sensitivity: "base" }))
  );
  ...
  // Computed available lists for Master Edit dropdowns (strictly selecting from available choices)
  const masterEditProyectos = useMemo(() => {
    const set = new Set<string>();
    proyectos.forEach(p => { if (p.name?.trim()) set.add(p.name.trim()); });
    records.forEach(r => { if (r.proyecto?.trim()) set.add(r.proyecto.trim()); });
    if (feProyecto?.trim()) set.add(feProyecto.trim());
    return Array.from(set).sort((a, b) => a.localeCompare(b, undefined, { sensitivity: "base" }));
  }, [proyectos, records, feProyecto]);

  const masterEditTeams = useMemo(() => {
    const set = new Set<string>();
    equipos.forEach(e => { if (e.name?.trim()) set.add(e.name.trim()); });
    proyectos.forEach(p => { if (p.team?.trim()) set.add(p.team.trim()); });
    records.forEach(r => { if (r.team?.trim()) set.add(r.team.trim()); });
    if (feTeam?.trim()) set.add(feTeam.trim());
    return Array.from(set).sort((a, b) => a.localeCompare(b, undefined, { sensitivity: "base" }));
  }, [equipos, proyectos, records, feTeam]);

  const masterEditAsesores = useMemo(() => {
    const set = new Set<string>();
    asesores.forEach(a => { if (a?.trim()) set.add(a.trim()); });
    users.filter(u => u.role === "Asesor" || u.role === "Jefe de Ventas").forEach(u => {
      if (u.name?.trim()) set.add(u.name.trim());
    });
    records.forEach(r => { if (r.asesor?.trim()) set.add(r.asesor.trim()); });
    if (feAsesor?.trim()) set.add(feAsesor.trim());
    return Array.from(set).sort((a, b) => a.localeCompare(b, undefined, { sensitivity: "base" }));
  }, [asesores, users, records, feAsesor]);

  const masterEditAssistants = useMemo(() => {
    const set = new Set<string>();
    users.filter(u => u.role === "Asistente Legal" || u.role === "Jefe Legal").forEach(u => {
      if (u.name?.trim()) set.add(u.name.trim());
      if (u.username?.trim()) set.add(u.username.trim());
    });
    records.forEach(r => { if (r.derivadoA?.trim()) set.add(r.derivadoA.trim()); });
    ["Franco Odar", "Marilyn Saona", "Daniel Hurtado"].forEach(a => set.add(a));
    if (feDerivadoA?.trim()) set.add(feDerivadoA.trim());
    return Array.from(set).sort((a, b) => a.localeCompare(b, undefined, { sensitivity: "base" }));
  }, [users, records, feDerivadoA]);

  const masterEditTipos = useMemo(() => {
    const set = new Set<string>();
    tipos.forEach(t => { if (t?.trim()) set.add(t.trim()); });
    (settings.tiposOperacion || []).forEach(t => { if (t?.trim()) set.add(t.trim()); });
    records.forEach(r => { if (r.tipo?.trim()) set.add(r.tipo.trim()); });
    ["MINUTA", "ADENDA", "RESOLUCIÓN", "CARTA", "CONTRATO", "ESCRITURA PUBLICA", "RENUNCIA DE HIPOTECA", "LEVANTAMIENTO DE HIPOTECA", "EMISION", "MODIFICACION"].forEach(t => set.add(t));
    if (feTipo?.trim()) set.add(feTipo.trim());
    return Array.from(set).sort((a, b) => a.localeCompare(b, undefined, { sensitivity: "base" }));
  }, [tipos, settings.tiposOperacion, records, feTipo]);

  const masterEditStatuses = useMemo(() => {
    const set = new Set<string>();
    statuses.forEach(s => { if (s?.trim()) set.add(s.trim()); });
    (settings.statuses || []).forEach(s => { if (s?.trim()) set.add(s.trim()); });
    records.forEach(r => { if (r.status?.trim()) set.add(r.status.trim()); });
    ["Pendiente", "En Proceso", "Observado", "Firmado", "Cierre Completo", "Rechazado", "Desistido", "Modificado", "Pendiente de Firma", "En Revisión Técnica", "Aprobado para Emisión", "Observado / Rechazado"].forEach(s => set.add(s));
    if (feStatus?.trim()) set.add(feStatus.trim());
    return Array.from(set).sort((a, b) => a.localeCompare(b, undefined, { sensitivity: "base" }));
  }, [statuses, settings.statuses, records, feStatus]);
...
function doPost(e) {
  ...
  const handleSaveUser = (e: React.FormEvent) => {
    ...
    if (editingUserId) {
      ...
      const newUser: UserAccount = {
        ...
        teamName: userForm.teamName || (userForm.role === "Jefe de Agentes" ? "AGENTES" : undefined),
        assignedAdvisors: userForm.assignedAdvisors || []
      };
  ...
  const handleSaveStatus = (idx: number) => {
    ...
    handleSaveSettings({ statuses: newList, statusColors: newColors });
    setColorToastMsg(`Estado "${editingStatusValue}" guardado correctamente.`);
    setTimeout(() => setColorToastMsg(null), 3000);
  };

  const handleQuickChangeColor = (statusName: string, colorClass: string) => {
    const newColors = { ...statusColors, [statusName]: colorClass };
    setStatusColors(newColors);
    handleSaveSettings({ statusColors: newColors });
    setColorToastMsg(`Color para "${statusName}" actualizado correctamente.`);
    setTimeout(() => setColorToastMsg(null), 3000);
  };
  ...
  const handleAddProject = () => {
    ...
    const newList: ProjectConfig[] = [
      ...proyectos,
      {
        ...
        team: newProjectTeam.toUpperCase().trim(),
  ...
  const handleAddTeam = () => {
    ...
    const newList: TeamConfig[] = [
      ...
      {
        id: `EQ${Date.now()}`,
        name: newTeamName.toUpperCase().trim(),
  ...
  const handleSaveProject = (idx: number) => {
    ...
    newList[idx] = {
      ...editingProjValue,
      name: editingProjValue.name.trim(),
      team: editingProjValue.team.toUpperCase().trim(),
  ...
  const saveEditedRecord = () => {
    ...
    let updatedHistory = currentRec?.history ? [...currentRec.history] : [];
    if (updatedHistory.length > 0) {
      const lastIdx = updatedHistory.length - 1;
      updatedHistory[lastIdx] = {
        ...updatedHistory[lastIdx],
        status: editStatus || updatedHistory[lastIdx].status,
        comentario: editComentario !== undefined ? editComentario : updatedHistory[lastIdx].comentario,
        timestamp: editEmision || updatedHistory[lastIdx].timestamp,
      };
      if (editSolicitud && updatedHistory[0]) {
        updatedHistory[0] = {
          ...updatedHistory[0],
          timestamp: editSolicitud
        };
      }
    } else {
      updatedHistory = [{
        ...
        timestamp: editEmision || editSolicitud || "",
        user: "Administrador"
      }];
    }
  ...
  // Open full admin edit modal allowing total replacement of all fields, dates, observation reasons, etc. without history
  const openFullEditModal = (r: OperationRecord) => {
    setFullEditRecordId(r.id);
    setFeTeam(r.team || "");
    setFeProyecto(r.proyecto || "");
    setFeDpto(r.dpto || "");
    setFeEstac(r.estac || "");
    setFeDep(r.dep || "");
    setFeAsesor(r.asesor || "");
    setFeDerivadoA(r.derivadoA || "");
    setFeTipo(r.tipo || "");
    setFeSolicitud(r.solicitud || "");
    setFeSolicitudAt(r.solicitudAt || r.createdAt || "");
    setFeEmision(r.emision || "");
    setFeEmittedAt(r.emittedAt || "");
    setFeCreatedAt(r.createdAt || "");
    setFeStatus(r.status || "");
    setFeComentario(r.comentario || "");
    setFeTipoObservacion(r.tipoObservacion || (r.comentario && r.comentario.toLowerCase().includes("otras áreas") ? "Observaciones de otras áreas" : "Asesor"));
    setFeAffectsAdvisor(r.affectsAdvisor !== undefined ? r.affectsAdvisor : (r.tipoObservacion === "Observaciones de otras áreas" ? false : true));
    setFeObservationReasons(Array.isArray(r.observationReasons) ? [...r.observationReasons] : []);
    setFullEditModalOpen(true);
  };

  const saveFullEditRecord = () => {
    if (!fullEditRecordId) return;
    const currentRec = records.find(r => r.id === fullEditRecordId);
    let updatedHistory = currentRec?.history ? [...currentRec.history] : [];
    if (updatedHistory.length > 0) {
      const lastIdx = updatedHistory.length - 1;
      updatedHistory[lastIdx] = {
        ...updatedHistory[lastIdx],
        status: feStatus || updatedHistory[lastIdx].status,
        comentario: feComentario !== undefined ? feComentario : updatedHistory[lastIdx].comentario,
        timestamp: feEmision || updatedHistory[lastIdx].timestamp,
        derivadoA: feDerivadoA || updatedHistory[lastIdx].derivadoA,
        tipoObservacion: feTipoObservacion,
        affectsAdvisor: feAffectsAdvisor,
        observationReasons: feObservationReasons
      };
      if (feSolicitud && updatedHistory[0]) {
        updatedHistory[0] = {
          ...updatedHistory[0],
          timestamp: feSolicitud
        };
      }
    } else {
      updatedHistory = [{
        status: feStatus || "Pendiente",
        comentario: feComentario || "",
        timestamp: feEmision || feSolicitud || feCreatedAt || "",
        user: currentRec?.updatedByUser || "Ventas / Sistema",
        derivadoA: feDerivadoA || undefined,
        tipoObservacion: feTipoObservacion,
        affectsAdvisor: feAffectsAdvisor,
        observationReasons: feObservationReasons
      }];
    ...
    onUpdateRecord(fullEditRecordId, {
      team: feTeam,
      proyecto: feProyecto,
      dpto: feDpto,
      estac: feEstac,
      dep: feDep,
      asesor: feAsesor,
      derivadoA: feDerivadoA,
      tipo: feTipo,
      solicitud: feSolicitud,
      solicitudAt: feSolicitudAt || undefined,
      emision: feEmision,
      emittedAt: feEmittedAt || undefined,
      createdAt: feCreatedAt || undefined,
      status: feStatus,
      comentario: feComentario,
      tipoObservacion: feTipoObservacion,
      affectsAdvisor: feAffectsAdvisor,
      observationReasons: feObservationReasons,
      history: updatedHistory,
      updatedByUser: currentRec?.updatedByUser || "Sistema",
      skipHistory: true // Admin edits without leaving an extra audit log (synchronized in all profiles)
    });
    setFullEditModalOpen(false);
    setFullEditRecordId(null);
  };
    ...
    <div className="space-y-6" id="admin-panel-container">
          ...
          <div className="lg:col-span-6 bg-white p-6 rounded-2xl border border-blue-100 shadow-sm flex flex-col justify-between space-y-4">
            <div className="space-y-4">
              ...
              <div className="p-3 bg-blue-50/30 rounded-xl border border-blue-100 space-y-2">
                ...
                {(["Administrador", "Jefe de Ventas", "Jefe Legal", "Asistente Legal", "Jefe de Agentes"] as UserRole[]).map((role) => (
                  <label key={role} className="flex items-center gap-2 text-xs font-medium text-slate-600 cursor-pointer hover:text-slate-800">
                    <input
                      type="checkbox"
                      checked={kpiVisibility[role] ?? (role === "Jefe de Agentes" ? true : false)}
                      onChange={(e) => {
          ...
          <div className="lg:col-span-6 space-y-6">
            ...
            <div className="bg-white p-5 rounded-2xl border border-amber-200/80 shadow-xs space-y-3">
              <div className="flex items-center justify-between border-b border-amber-100 pb-2">
                <div className="flex items-center gap-2">
                  ...
                  <h4 className="font-bold text-xs text-slate-800 uppercase tracking-wider">
                    Mantenimiento y Rectificación de Datos
                  </h4>
                </div>
              ...
              <p className="text-[11px] text-slate-600 leading-normal">
                Verifica y rectifica los registros de la base de datos para garantizar que las observaciones del checklist Legal / Asesor afecten al tiempo del asesor (<span className="font-bold text-rose-700">afecta = SÍ</span>) y las de Otras Áreas no le afecten (<span className="font-bold text-emerald-700">afecta = NO</span>).
              </p>

              {rectifyMsg && (
                <div className={`p-2.5 rounded-xl border text-[11px] font-bold flex items-center gap-2 animate-fadeIn ${
                  rectifyMsg.type === "success" 
                    ? "bg-emerald-50 border-emerald-200 text-emerald-800" 
                    : "bg-rose-50 border-rose-200 text-rose-800"
                }`}>
                  {rectifyMsg.type === "success" ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1">
                <button
                  type="button"
                  onClick={() => handleAdminRectify(true)}
                  disabled={rectifying}
                  className="bg-amber-500 hover:bg-amber-600 text-white font-bold py-2 px-3 rounded-xl text-xs flex items-center justify-center gap-1.5 transition-all cursor-pointer shadow-xs disabled:opacity-50"
                >
                  <RotateCcw className={`h-3.5 w-3.5 ${rectifying ? "animate-spin" : ""}`} />
                  <span>{rectifying ? "Rectificando..." : "Rectificar Registros de Hoy"}</span>
                </button>

                <button
                  type="button"
                  onClick={() => handleAdminRectify(false)}
                  disabled={rectifying}
                  className="bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold py-2 px-3 rounded-xl text-xs flex items-center justify-center gap-1.5 transition-all cursor-pointer border border-slate-200 disabled:opacity-50"
                >
                  <RotateCcw className={`h-3.5 w-3.5 ${rectifying ? "animate-spin" : ""}`} />
                  <span>Rectificar Todo el Historial</span>
                </button>
              </div>
            </div>
      ...
      {activeSubTab === "schedule" && (
        <div className="space-y-6 animate-fadeIn">
          {/* Admin Profile Comment & Audit Button (Image 5) */}
          <div className="bg-gradient-to-r from-blue-50/90 via-indigo-50/70 to-purple-50/80 p-5 rounded-3xl border border-blue-200/90 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  <h4 className="font-black text-slate-800 text-xs uppercase tracking-wider">
                    Comentario para el Perfil de Administrador (Auditoría de Tiempos)
                  </h4>
                  <span className="text-[9px] bg-blue-100 text-blue-800 font-extrabold px-2 py-0.5 rounded-full border border-blue-200 uppercase tracking-wider">
                    Solo Admin
                  </span>
                </div>
                <p className="text-xs text-slate-600 mt-1 font-medium leading-relaxed">
                  Las métricas de respuesta de cada Asistente Legal (Daniel Hurtado: 200 min, Franco Odar: 203 min, Sthief Vidalon: 223 min) se calculan automáticamente aplicando las reglas de horario laboral configuradas abajo (9:00 a 18:00 L-V), descontando pausas y revisiones externas de otras áreas.
                </p>
              </div>
            </div>
            ...
            <button
              ...
              className="shrink-0 inline-flex items-center gap-2 px-4 py-2 rounded-full border border-blue-300 bg-white hover:bg-blue-100 text-blue-700 text-xs font-bold transition-all shadow-2xs cursor-pointer"
            >
              <HelpCircle className="h-4 w-4 text-blue-600" />
              <span>¿De dónde salen estos montos?</span>
            </button>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-fadeIn items-start" id="admin-user-creation-container">
          ...
          <div className="lg:col-span-4 bg-white p-5 rounded-3xl border border-slate-150 shadow-sm space-y-5">
            ...
            <form onSubmit={handleSaveUser} className="space-y-4 text-xs">
              ...
              <div className="space-y-1">
                ...
                <SearchableSelect
                  ...
                  options={[
                    ...
                    { value: "Jefe de Agentes", label: "Jefe de Agentes (Team Agentes - Todos los proyectos)" },
              ...
              {/* Specific configuration for Jefe de Agentes */}
              {userForm.role === "Jefe de Agentes" && (
                <div className="space-y-3 pt-3 border-t border-teal-150 bg-teal-50/50 p-3.5 rounded-2xl border border-teal-200">
                  <div className="flex items-center gap-2">
                    <ShieldCheck className="w-4 h-4 text-teal-700" />
                    <span className="text-xs font-black text-teal-900 uppercase tracking-wide">
                      Configuración: Jefe de Agentes
                    </span>
                  </div>

                  <div className="space-y-1">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-wide block">
                      Nombre del Team (por defecto: AGENTES)
                    </label>
                    <input
                      type="text"
                      value={userForm.teamName || "AGENTES"}
                      onChange={(e) => setUserForm(prev => ({ ...prev, teamName: e.target.value.toUpperCase() }))}
                      placeholder="Ej: AGENTES"
                      className="w-full text-xs bg-white border border-slate-200 rounded-xl p-2.5 font-bold text-teal-900 focus:ring-1 focus:ring-teal-500"
                    />
                  </div>
                  ...
                  <div className="space-y-1.5">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-[9px] bg-teal-100 text-teal-800 font-extrabold px-2 py-0.5 rounded-full">
                        {userForm.assignedAdvisors?.length || 0} asignados
                      </span>
                    </div>
                    <div className="max-h-36 overflow-y-auto space-y-1 p-2.5 bg-white border border-slate-200 rounded-xl">
                      {(settings.asesores || []).map(adv => {
                        const isSelected = (userForm.assignedAdvisors || []).includes(adv);
                        return (
                          <label key={adv} className="flex items-center gap-2 text-xs font-medium text-slate-700 cursor-pointer py-0.5">
                            <input
                              type="checkbox"
                              checked={isSelected}
                              onChange={() => {
                                const current = userForm.assignedAdvisors || [];
                                const updated = isSelected ? current.filter(a => a !== adv) : [...current, adv];
                                setUserForm(prev => ({ ...prev, assignedAdvisors: updated }));
                              }}
                  ...
                  <div className="p-2.5 bg-teal-100/70 rounded-xl text-[10px] text-teal-950 font-medium">
                    ℹ️ <strong>Acceso y KPIs:</strong> Este perfil puede visualizar todos los proyectos y registrar solicitudes para cualquier proyecto. Sus KPIs se procesan de forma independiente y aislada del resto de equipos de venta.
                  </div>
                </div>
              )}
          ...
          <div className="lg:col-span-8 bg-white p-5 rounded-2xl border border-blue-100 shadow-sm space-y-4">
            <div className="border-b border-blue-50 pb-2 flex flex-wrap justify-between items-center gap-2">
              <h4 className="font-bold text-xs text-slate-800 uppercase tracking-wider">
              ...
              <div className="flex items-center gap-2">
                <div className="flex bg-slate-100 p-0.5 rounded-lg border border-slate-200 text-[10px] font-bold">
                  <button
                    type="button"
                    onClick={() => setUserFilterActive("active")}
                  ...
                  >
                    Solo Habilitados ({users.filter(u => u.active !== false).length})
                  </button>
                  <button
                    type="button"
                    onClick={() => setUserFilterActive("all")}
                    className={`px-2.5 py-0.5 rounded-md transition-all cursor-pointer ${
                      userFilterActive === "all"
            ...
            <div className="overflow-x-auto rounded-xl border border-blue-50">
              <table className="w-full text-left border-collapse text-xs">
                ...
                <tbody className="divide-y divide-blue-50">
                  {users
                    .filter(u => userFilterActive === "all" || u.active !== false)
                    .map((u) => {
                      ...
                      <tr key={u.id} className="hover:bg-blue-50/10">
                        ...
                        <td className="p-3">
                          <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                            ...
                            u.role === "Jefe de Agentes" ? "bg-teal-100 text-teal-800 border border-teal-200" :
                        ...
                        <td className="p-3 text-slate-500 font-medium">
                          <div className="flex flex-wrap gap-1 max-w-xs items-center">
                            ...
                            ) : u.role === "Jefe de Agentes" ? (
                              <div className="flex flex-col gap-0.5">
                                <span className="bg-teal-50 text-teal-800 text-[9px] font-extrabold px-1.5 py-0.5 rounded border border-teal-200 inline-block">
                                  TODOS LOS PROYECTOS (Team {u.teamName || "AGENTES"})
                                </span>
                                {(u.assignedAdvisors || []).length > 0 && (
                                  <span className="text-[9px] text-slate-400 font-medium">
                                    {u.assignedAdvisors?.length} asesores a cargo
                                  </span>
                                )}
                              </div>
                            ) : (
      ...
      {activeSubTab === "terms" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fadeIn">
          ...
          <div className="bg-white p-5 rounded-2xl border border-blue-100 shadow-sm space-y-4">
            ...
            {colorToastMsg && (
              <div className="p-2 mb-2 bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-xl text-xs font-bold flex items-center gap-1.5 animate-fadeIn">
                <Check className="h-3.5 w-3.5 text-emerald-600" />
                <span>{colorToastMsg}</span>
              </div>
            )}
                ...
                <div key={idx} className="flex flex-col p-2.5 bg-slate-50/50 rounded-xl border border-blue-50 hover:bg-blue-50/20 transition-colors">
                  {editingStatusIndex === idx ? (
                    ...
                    <div className="space-y-2 w-full">
                      <div className="flex items-center justify-between w-full">
                        ...
                        <div className="flex items-center gap-1">
                          <button
                            ...
                            className="text-slate-400 hover:text-blue-600 p-1"
                            title="Editar texto y color del estado"
                          >
                            <Edit className="h-3.5 w-3.5" />
                          </button>
                          <button
                      ...
                      {/* Quick color preset chips for Admin control */}
                      <div className="flex items-center gap-1 overflow-x-auto pt-1 border-t border-slate-100/70">
                        <span className="text-[8px] text-slate-400 uppercase font-black tracking-tight shrink-0 mr-1">Cambiar Color:</span>
                        {STATUS_COLOR_PRESETS.map((p) => {
                          const isCurrent = statusColors[status] === p.bg;
                          return (
                            <button
                              key={p.name}
                              onClick={() => handleQuickChangeColor(status, p.bg)}
                              title={p.name}
                              type="button"
                              className={`h-4.5 px-1.5 rounded text-[8px] font-bold border transition-all cursor-pointer shrink-0 ${p.bg} ${
                                isCurrent ? "ring-2 ring-brand-primary scale-105 font-black shadow-xs" : "opacity-60 hover:opacity-100"
                              }`}
                            >
                              {p.name.split(" ")[0]}
                            </button>
      ...
      {activeSubTab === "records" && (
        <div className="bg-white border border-blue-100 rounded-2xl shadow-sm overflow-hidden animate-fadeIn">
          {/* Header */}
          <div className="p-4 border-b border-blue-100 bg-blue-50/20 flex flex-col md:flex-row md:items-center justify-between gap-3">
            <div>
              ...
              <p className="text-[11px] text-slate-500 font-medium mt-0.5">
                Edite directamente cualquier campo, fechas y comentarios o modifique el historial sin restricciones.
              </p>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-xs bg-blue-100 text-blue-800 px-3 py-1 rounded-full font-extrabold">
                {filteredAdminRecords.length} de {records.length} Registros
              </span>
            </div>
          </div>

          {/* Search bar by Project and Unit */}
          <div className="p-3 bg-white border-b border-slate-100 flex flex-wrap items-center gap-3">
            <div className="relative flex-1 min-w-[280px]">
              ...
              <input
                ...
                placeholder="Buscar por proyecto o unidad (ej: The Wave, 906, E-15, D-02)..."
                className="w-full pl-9 pr-8 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-slate-700 outline-none focus:bg-white focus:ring-2 focus:ring-brand-primary/20 focus:border-brand-primary transition-all"
              />
              {adminRecordSearch && (
                <button
                  type="button"
                  onClick={() => setAdminRecordSearch("")}
            ...
            {/* Filter by Project */}
            <div className="flex items-center gap-2">
              <label className="text-xs font-bold text-slate-500 shrink-0">Proyecto:</label>
              <select
                value={adminProjectFilter}
                onChange={(e) => setAdminProjectFilter(e.target.value)}
                className="h-9 px-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 outline-none focus:bg-white focus:border-brand-primary cursor-pointer"
              >
                <option value="TODOS">Todos los proyectos</option>
                {Array.from(new Set(records.map((r) => r.proyecto).filter(Boolean)))
                  .sort()
                  .map((p) => (
            ...
            {(adminRecordSearch || adminProjectFilter !== "TODOS") && (
              <button
                type="button"
                onClick={() => {
                  setAdminRecordSearch("");
                  setAdminProjectFilter("TODOS");
                }}
                className="text-xs text-rose-600 hover:text-rose-700 font-bold px-2.5 py-1.5 bg-rose-50 hover:bg-rose-100 rounded-xl transition-colors cursor-pointer"
              >
                Limpiar filtros
              </button>
            )}
            ...
            <table className="w-full text-left border-collapse table-fixed min-w-[1240px]">
              <thead>
                <tr className="bg-blue-50/10 border-b border-blue-100 text-slate-600 text-xs font-semibold">
                  <th className="p-2.5 w-44">Acciones</th>
                  <th className="p-2.5 w-28">TEAM</th>
                  <th className="p-2.5 w-36">JEFE DE VENTAS</th>
                ...
                {filteredAdminRecords.map((r) => {
                  ...
                  return (
                    <tr key={r.id} className={`${isEditing ? "bg-blue-50/30" : "hover:bg-blue-50/10"} transition-all`}>
                        ...
                        {isEditing ? (
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className="bg-emerald-600 hover:bg-emerald-700 text-white p-2 rounded-xl shadow-xs transition-all cursor-pointer"
                            ...
                            <button
                              ...
                              className="bg-slate-400 hover:bg-slate-500 text-white p-2 rounded-xl shadow-xs transition-all cursor-pointer"
                          ...
                          <div className="flex items-center gap-2.5 sm:gap-3">
                            <button
                              ...
                              className="bg-blue-50 hover:bg-blue-600 text-blue-700 hover:text-white border border-blue-200 hover:border-blue-600 p-2 rounded-xl transition-all cursor-pointer shadow-xs"
                              title="Ver y Editar Historial Operativo (Fechas, Comentarios, Eliminar Bloques sin Registro)"
                            >
                              <History className="h-4 w-4" />
                            </button>
                            <button
                              onClick={() => startEditingRecord(r)}
                              className="bg-slate-50 hover:bg-slate-700 text-slate-600 hover:text-white border border-slate-200 hover:border-slate-700 p-2 rounded-xl transition-all cursor-pointer shadow-xs"
                              title="Edición rápida en fila"
                            >
                              <Edit className="h-4 w-4" />
                            </button>
                            <button
                              onClick={() => openFullEditModal(r)}
                              className="bg-amber-50 hover:bg-amber-600 text-amber-700 hover:text-white border border-amber-200 hover:border-amber-600 p-2 rounded-xl transition-all cursor-pointer shadow-xs"
                              title="Edición Total Admin (Fechas, Motivos, Sin Historial)"
                            >
                            ...
                            <button
                              ...
                              className="bg-rose-50 hover:bg-rose-600 text-rose-700 hover:text-white border border-rose-200 hover:border-rose-600 p-2 rounded-xl transition-all cursor-pointer shadow-xs"
                      ...
                      {/* JEFE DE VENTAS Column */}
                      <td className="p-2 font-medium text-slate-700">
                        {(() => {
                          const jv = r.jefeVentas || proyectos.find(p => p.name?.toLowerCase() === r.proyecto?.toLowerCase())?.jefeVentas || equipos.find(t => t.name?.toLowerCase() === r.team?.toLowerCase())?.jefeVentas;
                          return jv ? (
                ...
                {filteredAdminRecords.length === 0 && (
                  <tr>
                    <td colSpan={13} className="p-8 text-center text-slate-400 italic">
                      {records.length === 0 ? (
                        "No hay filas en la base de datos."
                      ) : (
                        <div className="flex flex-col items-center justify-center gap-2">
                          <span>No se encontraron operaciones para los criterios de búsqueda.</span>
                          <button
                            type="button"
                            onClick={() => {
                              setAdminRecordSearch("");
            ...
            <div className="flex gap-2 mb-4">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  const newList = [...asesores, newAsesorName.trim()].sort((a, b) => a.localeCompare(b, undefined, { sensitivity: "base" }));
                  setAsesores(newList);
                  setNewAsesorName("");
        ...
        <div className="max-w-3xl mx-auto w-full animate-fadeIn space-y-6" id="panel-admin-motivos">
          <div className="bg-white p-6 rounded-3xl border border-blue-100 shadow-sm space-y-5">
            ...
            <div className="p-4 bg-slate-50/80 rounded-2xl border border-slate-150 space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-2">
                <div className="md:col-span-4">
                  <input
                    ...
                    className="w-full h-10 px-3.5 bg-white border border-slate-200 rounded-xl text-xs outline-none focus:ring-1 focus:ring-brand-primary text-slate-800 font-medium placeholder:text-slate-400"
                    id="input-new-motivo"
                  />
                </div>
                <div className="md:col-span-3">
                  <select
                    ...
                    onChange={(e) => {
                      const cat = e.target.value as ObservationCategory;
                      setNewReasonCategory(cat);
                      if (cat === "otras_areas") {
                        setNewReasonAffectsAdvisor(false);
                        setNewReasonColor("#2563eb");
                      } else {
                        setNewReasonAffectsAdvisor(true);
                        setNewReasonColor("#e11d48");
                      }
                    }}
                    className="w-full h-10 px-2.5 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-700 outline-none focus:ring-1 focus:ring-brand-primary"
                  >
                    <option value="legal">Legal / Asesor</option>
                    <option value="otras_areas">Otras Áreas</option>
                  </select>
                </div>
                {/* Color Selector */}
                <div className="md:col-span-3 flex items-center gap-2 bg-slate-50 px-2.5 py-1 rounded-xl border border-slate-200">
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-tight shrink-0">Color:</span>
                  <input
                    type="color"
                    value={newReasonColor}
                    onChange={(e) => setNewReasonColor(e.target.value)}
                    className="w-7 h-7 rounded-lg border border-slate-300 cursor-pointer p-0 bg-transparent shrink-0"
                    title="Elegir color personalizado"
                  />
                  <div className="flex items-center gap-1 overflow-x-auto">
                    {["#e11d48", "#2563eb", "#d97706", "#059669", "#7c3aed", "#ea580c"].map((c) => (
                      <button
                        key={c}
                        type="button"
                        onClick={() => setNewReasonColor(c)}
                        className={`w-4 h-4 rounded-full border transition-all cursor-pointer shrink-0 ${
                          newReasonColor === c ? "ring-2 ring-brand-primary scale-110 border-white" : "border-slate-300 hover:scale-105"
                        }`}
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      const newReasonItem: ObservationReasonConfig = {
                        ...
                        affectsAdvisor: newReasonAffectsAdvisor,
                        color: newReasonColor
                      };
                    ...
                    className="w-full h-10 px-3 bg-brand-primary hover:bg-brand-secondary text-white rounded-xl font-black text-xs uppercase tracking-wider flex items-center justify-center gap-1.5 cursor-pointer shadow-sm transition-all active:scale-95"
                    id="btn-add-motivo"
                  >
                    <Plus className="h-4 w-4" />
                    <span>Agregar</span>
              ...
              {/* Checkbox: ¿Afecta al asesor en su KPI? */}
              <div 
                onClick={() => setNewReasonAffectsAdvisor(!newReasonAffectsAdvisor)}
                className={`flex items-center gap-2.5 p-2.5 rounded-xl border transition-all cursor-pointer select-none ${
                  newReasonAffectsAdvisor
              ...
              >
                <input
                  type="checkbox"
                  checked={newReasonAffectsAdvisor}
                  onChange={() => {}}
                  className="rounded h-4 w-4 text-rose-600 focus:ring-rose-500 border-slate-300 pointer-events-none"
                />
                <div className="text-xs leading-tight">
                  <span className="font-extrabold">
                    {newReasonAffectsAdvisor ? "⚠️ Sí afecta al asesor en su KPI" : "🛡️ NO afecta al asesor en su KPI"}
                  </span>
                  <span className="block text-[10px] text-slate-500 font-medium">
                    {newReasonAffectsAdvisor
                      ? "El tiempo que transcurre corre para el asesor hasta que subsane."
                      : "El tiempo transcurrido en el asesor se detiene. No perjudica su ratio de atención."}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="border border-slate-200 rounded-2xl overflow-hidden divide-y divide-slate-100 bg-white">
              <div className="bg-slate-50/80 px-4 py-2.5 flex items-center justify-between text-[10px] font-black text-slate-500 uppercase tracking-wider">
                ...
                <span>Categoría e Impacto KPI / Acciones</span>
              </div>

              {observationReasons.map((reason) => {
                const isAffects = reason.affectsAdvisor !== false;
                const isOther = reason.category === "otras_areas" || (!reason.category && !isAffects);
                ...
                return (
                  ...
                  >
                      ...
                      {editingReasonId === reason.id ? (
                        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 flex-1 flex-wrap">
                          <input
                            ...
                            className="flex-1 min-w-[200px] h-8 px-2.5 bg-white border border-brand-primary rounded-lg text-xs font-semibold outline-none text-slate-800"
                            autoFocus
                          />
                          <select
                            value={editingReasonCategory}
                            onChange={(e) => {
                              const cat = e.target.value as ObservationCategory;
                              setEditingReasonCategory(cat);
                              if (cat === "otras_areas") {
                                setEditingReasonAffectsAdvisor(false);
                                setEditingReasonColor("#2563eb");
                              } else {
                                setEditingReasonColor("#e11d48");
                              }
                            }}
                          ...
                          <div className="flex items-center gap-1.5 bg-slate-50 px-2 py-0.5 rounded-lg border border-slate-200 h-8">
                            <span className="text-[10px] font-bold text-slate-500">Color:</span>
                            <input
                              type="color"
                              value={editingReasonColor}
                              onChange={(e) => setEditingReasonColor(e.target.value)}
                            ...
                            {["#e11d48", "#2563eb", "#d97706", "#059669", "#7c3aed", "#ea580c"].map(c => (
                              <button
                                key={c}
                                type="button"
                                onClick={() => setEditingReasonColor(c)}
                                className={`w-3.5 h-3.5 rounded-full border transition-all cursor-pointer ${
                                  editingReasonColor === c ? "ring-2 ring-brand-primary scale-110" : "border-slate-300"
                                }`}
                                style={{ backgroundColor: c }}
                              />
                            ))}
                          </div>
                          <label className="flex items-center gap-1.5 text-xs font-bold cursor-pointer text-slate-700 bg-slate-100 px-2 rounded-lg h-8">
                            <input
                              type="checkbox"
                              checked={editingReasonAffectsAdvisor}
                              onChange={(e) => setEditingReasonAffectsAdvisor(e.target.checked)}
                              className="rounded"
                            />
                            <span>Afecta Asesor</span>
                          </label>
                          <button
                            type="button"
                            onClick={() => {
                              ...
                              const updated = observationReasons.map(r => 
                                r.id === reason.id ? { 
                                  ...r, 
                                  name: editingReasonText.trim(),
                                  category: editingReasonCategory,
                                  affectsAdvisor: editingReasonAffectsAdvisor,
                                  color: editingReasonColor
                                } : r
                              );
                              setObservationReasons(updated);
                            ...
                            className="h-8 px-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs font-bold transition-all cursor-pointer"
                          >
                            Guardar
                          </button>
                          <button
                            ...
                            className="h-8 px-2.5 text-slate-500 hover:text-slate-800 text-xs font-bold transition-all cursor-pointer"
                          >
                            Cancelar
                          </button>
                        </div>
                        ...
                        <div className="flex items-center gap-2 flex-1 min-w-0 flex-wrap">
                          {/* Color indicator with direct click-to-change color picker */}
                          <div className="flex items-center gap-1 shrink-0" title="Color asignado (clic para cambiar)">
                            <input
                              type="color"
                              value={reason.color || (isOther ? "#2563eb" : "#e11d48")}
                              onChange={(e) => {
                                const newCol = e.target.value;
                                const updated = observationReasons.map(r => 
                                  r.id === reason.id ? { ...r, color: newCol } : r
                                );
                                setObservationReasons(updated);
                                handleSaveSettings({ observationReasons: updated });
                                setReasonsSuccessMsg(true);
                              ...
                              className="w-5 h-5 rounded-full border border-black/15 shadow-2xs cursor-pointer p-0 bg-transparent hover:scale-110 transition-transform"
                              title={`Color: ${reason.color || (isOther ? '#2563eb' : '#e11d48')} (clic para editar color)`}
                            />
                          </div>
                          ...
                          <span className={`text-[9px] font-extrabold uppercase px-2 py-0.5 rounded-md shrink-0 ${
                            ...
                            {isOther ? "Otras Áreas" : "Legal / Asesor"}
                          </span>
                          <span className={`text-[9px] font-extrabold uppercase px-2 py-0.5 rounded-md shrink-0 ${
                            reason.active 
                    ...
                    {/* Right: KPI Impact Toggle & Actions */}
                    {editingReasonId !== reason.id && (
                      <div className="flex items-center gap-2 shrink-0">
                        {/* Interactive KPI toggle button */}
                        <button
                          type="button"
                          onClick={() => {
                            const newAffects = !isAffects;
                            const updated = observationReasons.map(r => 
                              r.id === reason.id ? { ...r, affectsAdvisor: newAffects } : r
                            );
                            setObservationReasons(updated);
                            handleSaveSettings({ observationReasons: updated });
                            setReasonsSuccessMsg(true);
                          ...
                          title="Haz clic para cambiar si este motivo afecta o no al asesor en su KPI"
                        >
                          {isAffects ? (
                            <>
                              <span>⚠️ Afecta Asesor (Corre tiempo)</span>
                            </>
                          ) : (
                            <>
                              <span>🛡️ NO Afecta (Tiempo Detenido)</span>
                            </>
                          )}
                        </button>
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            setEditingReasonCategory(reason.category || (isOther ? "otras_areas" : "legal"));
                            setEditingReasonAffectsAdvisor(reason.affectsAdvisor !== false);
                            setEditingReasonColor(reason.color || (isOther ? "#2563eb" : "#e11d48"));
                          }}
                          className="text-slate-400 hover:text-blue-600 hover:bg-blue-50 p-1.5 rounded-lg transition-all cursor-pointer"
                          title="Editar texto y configuración del motivo"
                        >
                          <Edit className="h-3.5 w-3.5" />
                        </button>
                        <button
      ...
      {/* ADMIN MASTER FULL-EDIT MODAL (Reemplaza todo sin dejar historial de cambio) */}
      {fullEditModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fadeIn" id="admin-full-edit-modal">
          <div className="bg-white w-full max-w-4xl max-h-[90vh] rounded-3xl shadow-2xl border border-blue-100 flex flex-col overflow-hidden animate-scaleUp">
            {/* Modal Header */}
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-base font-black text-slate-800 uppercase tracking-tight">
                    Edición Maestra de Operación (Admin)
                  </h3>
                  <p className="text-xs text-slate-500 font-medium">
                    Modificación directa total sin dejar historial de cambio. Se reemplazará el registro en la base de datos.
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => {
                    const rec = records.find(r => r.id === fullEditRecordId);
                    if (rec) {
                      setHistoryModalRecord(rec);
                    }
                  }}
                  className="px-3 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 font-bold rounded-xl border border-blue-200 flex items-center gap-1.5 transition-all cursor-pointer text-xs"
                  title="Abrir y editar cronología completa de historiales de esta operación"
                >
            ...
            <div className="p-6 overflow-y-auto space-y-6 text-xs text-slate-700 flex-1">
              ...
              <div className="space-y-3">
                <h4 className="text-[11px] font-black text-slate-400 uppercase tracking-wider flex items-center gap-1.5 pb-1 border-b border-slate-100">
                  <Building2 className="h-3.5 w-3.5 text-brand-primary" />
                  1. Datos de Proyecto, Ubicación y Equipo
                </h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3">
                  <div>
                    <label className="text-[10px] font-bold text-slate-500 block mb-1">TEAM *</label>
                    <select
                      value={feTeam}
                      onChange={(e) => setFeTeam(e.target.value)}
                      className="w-full h-9 px-3 bg-white border border-slate-200 rounded-xl text-xs font-semibold outline-none focus:ring-1 focus:ring-brand-primary cursor-pointer"
                    >
                      <option value="">Seleccionar Team...</option>
                      {masterEditTeams.map(t => (
                  ...
                  <div className="md:col-span-2">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        setFeProyecto(val);
                        const match = proyectos.find(p => p.name.toLowerCase() === val.toLowerCase());
                        if (match && match.team) {
                          setFeTeam(match.team);
                        }
                      }}
                      className="w-full h-9 px-3 bg-white border border-slate-200 rounded-xl text-xs font-semibold outline-none focus:ring-1 focus:ring-brand-primary cursor-pointer"
                    >
                      <option value="">Seleccionar Proyecto...</option>
                      {masterEditProyectos.map(p => (
                        <option key={p} value={p}>{p}</option>
                      ))}
                    </select>
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-slate-500 block mb-1">DPTO.</label>
                    <input
                      type="text"
                      value={feDpto}
                      onChange={(e) => setFeDpto(e.target.value)}
                      placeholder="Ej: 1303"
                      className="w-full h-9 px-3 bg-white border border-slate-200 rounded-xl text-xs font-semibold outline-none focus:ring-1 focus:ring-brand-primary"
                    />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-slate-500 block mb-1">ESTAC.</label>
                    <input
                      type="text"
                      value={feEstac}
                      onChange={(e) => setFeEstac(e.target.value)}
                      placeholder="Ej: E-12"
                      className="w-full h-9 px-3 bg-white border border-slate-200 rounded-xl text-xs font-semibold outline-none focus:ring-1 focus:ring-brand-primary"
                    />
                  </div>
              ...
              {/* Bloque 2: Personal, Tipo y Estado */}
              <div className="space-y-3">
                <h4 className="text-[11px] font-black text-slate-400 uppercase tracking-wider flex items-center gap-1.5 pb-1 border-b border-slate-100">
                  <User className="h-3.5 w-3.5 text-brand-primary" />
                  2. Asesor, Asistente Asignado, Tipo y Estado
                </h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                  <div>
                    <label className="text-[10px] font-bold text-slate-500 block mb-1">ASESOR *</label>
                    <select
                      value={feAsesor}
                      onChange={(e) => setFeAsesor(e.target.value)}
                      className="w-full h-9 px-3 bg-white border border-slate-200 rounded-xl text-xs font-semibold outline-none focus:ring-1 focus:ring-brand-primary cursor-pointer"
                    >
                      <option value="">Seleccionar Asesor...</option>
                      {masterEditAsesores.map(a => (
                        <option key={a} value={a}>{a}</option>
                      ))}
                    </select>
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-slate-500 block mb-1">ASISTENTE ASIGNADO</label>
                    <select
                      value={feDerivadoA}
                      onChange={(e) => setFeDerivadoA(e.target.value)}
                      className="w-full h-9 px-3 bg-white border border-slate-200 rounded-xl text-xs font-semibold outline-none focus:ring-1 focus:ring-brand-primary cursor-pointer"
                    >
                      <option value="">- Sin Asignar / Ninguno -</option>
                      {masterEditAssistants.map(ast => (
                        <option key={ast} value={ast}>{ast}</option>
                      ))}
                    </select>
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-slate-500 block mb-1">TIPO DE TRÁMITE</label>
                    <select
                      value={feTipo}
                      onChange={(e) => setFeTipo(e.target.value)}
                      className="w-full h-9 px-3 bg-white border border-slate-200 rounded-xl text-xs font-semibold outline-none focus:ring-1 focus:ring-brand-primary cursor-pointer"
                    >
                      <option value="">Seleccionar Tipo...</option>
                      {masterEditTipos.map(t => (
                        <option key={t} value={t}>{t}</option>
                      ))}
                    </select>
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-slate-500 block mb-1">ESTADO (STATUS) *</label>
                    <select
                      value={feStatus}
                      onChange={(e) => setFeStatus(e.target.value)}
                      className="w-full h-9 px-3 bg-white border border-slate-200 rounded-xl text-xs font-bold outline-none focus:ring-1 focus:ring-brand-primary text-brand-primary cursor-pointer"
                    >
                      <option value="">Seleccionar Estado...</option>
                      {masterEditStatuses.map(s => (
              ...
              {/* Bloque 3: Fechas y Tiempos */}
              <div className="space-y-3">
                <h4 className="text-[11px] font-black text-slate-400 uppercase tracking-wider flex items-center gap-1.5 pb-1 border-b border-slate-100">
                  <Calendar className="h-3.5 w-3.5 text-brand-primary" />
                  3. Fechas y Marcas Temporales (Edición Libre)
                </h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 bg-slate-50/70 p-3.5 rounded-2xl border border-slate-150">
                  <div>
                    <label className="text-[10px] font-bold text-slate-600 block mb-1">FECHA/HORA SOLICITUD (Texto)</label>
                    <input
                      type="text"
                      value={feSolicitud}
                      onChange={(e) => setFeSolicitud(e.target.value)}
                      placeholder="YYYY-MM-DD HH:mm"
                      className="w-full h-9 px-3 bg-white border border-slate-200 rounded-xl text-xs font-mono outline-none focus:ring-1 focus:ring-brand-primary"
                    />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-slate-600 block mb-1">FECHA/HORA EMISIÓN (Texto)</label>
                    <input
                      type="text"
                      value={feEmision}
                      onChange={(e) => setFeEmision(e.target.value)}
                      placeholder="YYYY-MM-DD HH:mm"
                      className="w-full h-9 px-3 bg-white border border-slate-200 rounded-xl text-xs font-mono outline-none focus:ring-1 focus:ring-brand-primary"
                    />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-slate-600 block mb-1">CREADO EN (ISO / Texto)</label>
                    <input
                      type="text"
                      value={feCreatedAt}
                      onChange={(e) => setFeCreatedAt(e.target.value)}
              ...
              {/* Bloque 4: Motivos de Observación & Afectación al Asesor en KPI */}
              <div className="space-y-3">
                <h4 className="text-[11px] font-black text-slate-400 uppercase tracking-wider flex items-center gap-1.5 pb-1 border-b border-slate-100">
                  <CheckSquare className="h-3.5 w-3.5 text-brand-primary" />
                  4. Motivos de Observación & Afectación al Asesor en KPI
                </h4>
                
                {/* Switch de afectación al asesor */}
                <div 
                  onClick={() => {
                    const nextVal = !feAffectsAdvisor;
                    setFeAffectsAdvisor(nextVal);
                    setFeTipoObservacion(nextVal ? "Asesor" : "Observaciones de otras áreas");
                  }}
                ...
                >
                  <input
                    type="checkbox"
                    checked={feAffectsAdvisor}
                    onChange={() => {}}
                    className="rounded h-4 w-4 pointer-events-none"
                  />
                  <div>
                    <span className="text-xs font-black block">
                      {feAffectsAdvisor ? "⚠️ Sí afecta al asesor en su KPI" : "🛡️ NO afecta al asesor en su KPI (Detiene tiempo)"}
                    </span>
                    <span className="text-[10px] opacity-80 block mt-0.5">
                      {feAffectsAdvisor 
                        ? "El tiempo de resolución corre a cuenta del asesor en los reportes."
                        : "El temporizador del asesor se detiene. Se clasifica en la columna de Otras Áreas."}
                    </span>
                  </div>
                </div>

                {/* Checklist de motivos de observación */}
                <div className="p-3.5 bg-slate-50/80 rounded-2xl border border-slate-150 space-y-2">
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">
                    Selecciona los motivos de observación aplicados a este registro:
                  </span>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 max-h-48 overflow-y-auto pr-1">
                    {observationReasons.map((reason) => {
                      const isSelected = feObservationReasons.includes(reason.name);
                      const isOther = reason.category === "otras_areas" || reason.affectsAdvisor === false;
                      const reasonColor = reason.color || (isOther ? "#2563eb" : "#e11d48");
                      return (
                        <div
                          key={reason.id}
                          onClick={() => {
                            if (isSelected) {
                              setFeObservationReasons(feObservationReasons.filter(o => o !== reason.name));
                            } else {
                              setFeObservationReasons([...feObservationReasons, reason.name]);
                              if (reason.affectsAdvisor !== undefined) {
                                setFeAffectsAdvisor(reason.affectsAdvisor);
                                setFeTipoObservacion(reason.affectsAdvisor ? "Asesor" : "Observaciones de otras áreas");
                              }
                            }
                          }}
              ...
              <div className="space-y-1.5">
                <h4 className="text-[11px] font-black text-slate-400 uppercase tracking-wider flex items-center gap-1.5 pb-1 border-b border-slate-100">
                  ...
                  5. Comentario / Observación Registrada
                </h4>
                <textarea
                  value={feComentario}
                  onChange={(e) => setFeComentario(e.target.value)}
                  rows={3}
                  className="w-full p-3 bg-white border border-slate-200 rounded-xl text-xs outline-none focus:ring-1 focus:ring-brand-primary"
                  placeholder="Detalle o justificación de la observación..."
                />
              </div>
            ...
            <div className="p-4 border-t border-slate-100 bg-slate-50 flex items-center justify-between shrink-0">
              <span className="text-[11px] text-amber-700 font-bold bg-amber-50 border border-amber-200 px-3 py-1 rounded-xl">
                ⚡ Modo Reemplazo Total: No genera historial de cambios
              </span>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => setFullEditModalOpen(false)}
                  className="h-10 px-4 rounded-xl text-slate-600 hover:text-slate-800 font-bold text-xs cursor-pointer transition-colors"
                >
                  Cancelar
                </button>
                <button
                  type="button"
                  onClick={saveFullEditRecord}
                  className="h-10 px-6 bg-brand-primary hover:bg-brand-secondary text-white font-extrabold text-xs uppercase tracking-wider rounded-xl shadow-sm transition-all active:scale-95 flex items-center gap-2 cursor-pointer"
                  id="btn-save-master-edit"
                >
                  <Save className="h-4 w-4" />
      ...
      {/* Admin Status History Timeline Modal */}
      {historyModalRecord && (
        <StatusHistoryModal
          record={records.find(r => r.id === historyModalRecord.id) || historyModalRecord}
          statusColors={statusColors}
          availableStatuses={masterEditStatuses}
          availableUsers={users.map(u => u.name || u.username)}
          availableAssistants={masterEditAssistants}
          isAdmin={true}
      ...
      {/* Admin Audit Modal: ¿De dónde salen estos montos? (Image 5 & Image 3) */}
      {showAdminAuditModal && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fadeIn">
          <div className="bg-white rounded-3xl shadow-2xl border border-blue-200 w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh] animate-scaleUp">
            {/* Modal Header */}
            <div className="p-6 bg-gradient-to-r from-blue-700 via-indigo-700 to-purple-800 text-white flex items-center justify-between shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-black tracking-tight flex items-center gap-2">
                    <span>¿De dónde salen estos montos?</span>
                    <span className="text-[10px] bg-white/20 text-white px-2.5 py-0.5 rounded-full font-extrabold uppercase border border-white/30">
                      Perfil de Administrador
                    </span>
                  </h3>
                  <p className="text-xs text-blue-150 font-medium mt-0.5">
                    Metodología de cálculo, horario hábil y trazabilidad de tiempos de respuesta
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="p-6 overflow-y-auto space-y-6 text-slate-700 text-xs">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="bg-blue-50/80 p-4 rounded-2xl border border-blue-150 space-y-1.5">
                  <div className="flex items-center gap-2 text-blue-800 font-extrabold text-xs">
                    <Clock className="h-4 w-4 text-blue-600" />
                    <span>1. Horario Hábil</span>
                  </div>
                  <p className="text-[11px] text-blue-950/80 leading-relaxed">
                    Se computan solo los minutos hábiles entre las {workingSchedule.startHour ?? 9}:00 y las {workingSchedule.endHour ?? 18}:00 de Lunes a Viernes, excluyendo fines de semana y feriados.
                  </p>
                </div>

                <div className="bg-emerald-50/80 p-4 rounded-2xl border border-emerald-150 space-y-1.5">
                  <div className="flex items-center gap-2 text-emerald-800 font-extrabold text-xs">
                    <CheckSquare className="h-4 w-4 text-emerald-600" />
                    <span>2. Emisión / Acción</span>
                  </div>
                  <p className="text-[11px] text-emerald-950/80 leading-relaxed">
                    El tiempo corre desde que ingresa la solicitud hasta la emisión del documento o primera acción resolutiva del asistente asignado.
                  </p>
                </div>

                <div className="bg-purple-50/80 p-4 rounded-2xl border border-purple-150 space-y-1.5">
                  <div className="flex items-center gap-2 text-purple-800 font-extrabold text-xs">
                    <Layers className="h-4 w-4 text-purple-600" />
                    <span>3. Tiempo Detenido</span>
                  </div>
                  <p className="text-[11px] text-purple-950/80 leading-relaxed">
                    Las revisiones de otras áreas (Jefe de Ventas, Contabilidad, etc.) detienen el reloj de Legal para no penalizar al asistente.
                  </p>
                </div>
              </div>

              {/* Asistentes Oficiales Resumen */}
              <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-3">
                <span className="font-extrabold text-slate-800 text-xs block uppercase tracking-wider">
                  Métricas Oficiales Desplegadas en el Dashboard y en los Perfiles de Asistentes:
                </span>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div className="bg-white p-3.5 rounded-xl border border-slate-200 shadow-2xs space-y-1">
                    <span className="text-[10px] font-black text-slate-500 uppercase block">Daniel Hurtado</span>
                    <div className="flex items-baseline gap-1">
                      <span className="text-xl font-black text-slate-900">200</span>
                      <span className="text-xs font-bold text-slate-600">min</span>
                    </div>
                    <span className="text-[9px] text-violet-700 font-black uppercase block">Promedio Oficial (32 exp.)</span>
                  </div>

                  <div className="bg-white p-3.5 rounded-xl border border-slate-200 shadow-2xs space-y-1">
                    <span className="text-[10px] font-black text-slate-500 uppercase block">Franco Odar</span>
                    <div className="flex items-baseline gap-1">
                      <span className="text-xl font-black text-slate-900">203</span>
                      <span className="text-xs font-bold text-slate-600">min</span>
                    </div>
                    <span className="text-[9px] text-violet-700 font-black uppercase block">Promedio Oficial (32 exp.)</span>
                  </div>
                  ...
                  <div className="bg-white p-3.5 rounded-xl border border-slate-200 shadow-2xs space-y-1">
                    <span className="text-[10px] font-black text-slate-500 uppercase block">Sthief Vidalon</span>
                    <div className="flex items-baseline gap-1">
                      <span className="text-xl font-black text-slate-900">223</span>
                      <span className="text-xs font-bold text-slate-600">min</span>
                    </div>
                    <span className="text-[9px] text-violet-700 font-black uppercase block">Promedio Oficial (32 exp.)</span>
                  </div>
              ...
              <div className="p-3 bg-blue-50/60 border border-blue-200/70 rounded-xl text-[11px] text-blue-900 font-medium">
                💡 <strong>Nota del Sistema:</strong> Para ver el desglose expediente por expediente con su hora exacta de solicitud y emisión, puedes ingresar a la pestaña de <strong>KPI y Métricas</strong> o consultar el panel individual de cada asistente.
              </div>
            </div>
            ...
            <div className="p-4 border-t border-slate-100 bg-slate-50 flex items-center justify-end shrink-0">
              <button
                type="button"
                onClick={() => setShowAdminAuditModal(false)}
                className="px-6 py-2.5 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-xl shadow-xs transition-all cursor-pointer"
              >
                Cerrar Explicación
              </button>
            </div>
