...
export default function SearchableSelect({
    ...
    function handleClickOutside(event: MouseEvent) {
      if (containerRef.current && !containerRef.current.contains(event.target as Node)) {
        ...
        if (matched) {
          ...
          // If user typed part of a valid project name and clicked away, select the top matched option
          onChange(filteredOptions[0].value);
          setSearchTerm(filteredOptions[0].label);
        } else {
          // Reset search term strictly to the valid selected option label
  ...
  }, [selectedOption, searchTerm, allowCustom, filteredOptions, options, onChange]);
