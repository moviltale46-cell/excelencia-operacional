...
export default function JefeVentasPanel({ 
  ...
  // Auto-set first project option once as default on initial load
  const hasInitializedProj = useRef(false);
  useEffect(() => {
    if (projectOptions.length > 0 && !hasInitializedProj.current && !selectedProyecto) {
      setSelectedProyecto(projectOptions[0]);
      hasInitializedProj.current = true;
    }
  }, [projectOptions]);
  ...
  const filteredRecords = records.filter(r => {
    if (viewScope === "mis_proyectos") {
      ...
      // Requirement: Operations from Team Agentes belong to a separate team and must not affect Jefe de Ventas
      const isAgente = (r.team || "").toUpperCase().includes("AGENTE");
      if (isAgente) return false;
    }
  ...
  // Sort oldest first (arriba la operación más antigua)
  const sortedRecords = [...filteredRecords].sort((a, b) => {
    const tA = safeGetTime(a.solicitudAt || a.createdAt || a.solicitud) || 0;
    const tB = safeGetTime(b.solicitudAt || b.createdAt || b.solicitud) || 0;
    return tA - tB;
  });
    ...
    <div className="space-y-6" id="jefe-ventas-container">
      ...
      <section className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-white p-5 rounded-2xl border border-blue-100 shadow-vibrant flex flex-col gap-1">
          ...
          <span className="text-brand-primary font-black text-3xl">{sortedRecords.length}</span>
        </div>
      </section>
            ...
            <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-4">
                ...
                if (dupCheck?.isDuplicate) {
                  return (
                    <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl text-amber-800 text-[11px] flex items-start gap-2">
                      <span className="font-bold shrink-0 text-amber-900">⚠️</span>
                      <div>
              ...
              <div className="flex gap-3 mt-4">
                ...
                <button
                  ...
                  className={`flex-1 h-10 rounded-xl text-xs font-bold transition-all hover:shadow cursor-pointer ${
                    isSubmitting 
