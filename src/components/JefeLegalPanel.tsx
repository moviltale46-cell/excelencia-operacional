...
export default function JefeLegalPanel({ 
  ...
  const handleConfirmSummaryAction = () => {
    if (pendingActionType === "emit") {
      ...
      const targetRec = records.find(r => r.id === reopeningRecordId);
      if (targetRec) {
  ...
  const executeReopenWithAction = async (rec: OperationRecord) => {
    ...
    try {
      ...
      const historyComment = `[Reapertura de Operación - ${finalActionType}] Estado reactivado de "${rec.status}" a "${reopenNewStatus}". ${assignedAssistant ? `Reasignado a ${assignedAssistant}. ` : ""}Detalle: ${finalComment}`.trim();
      ...
      setCustomActionType("");
      setAssignedAssistant("");
      setToastMessage(`Operación ${formatProjectAndUnits(rec)} reabierta exitosamente con estado "${reopenNewStatus}".`);
      setTimeout(() => setToastMessage(null), 4000);
    } catch (e: any) {
                ...
                (() => {
                  ...
                  return (
                    <div className="bg-white p-6 rounded-3xl border border-slate-150 shadow-sm space-y-5">
                      ...
                      {/* Prompt to lift observation if currently observed in Jefe Legal evaluation form */}
                      {isCurrentObserved && currentRec && (
                        <div className="p-4 bg-emerald-50/90 border border-emerald-300 rounded-2xl space-y-3 overflow-hidden">
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                            <div className="flex items-start gap-2.5 text-emerald-950 flex-1 min-w-0">
                              <CheckCircle2 className="h-4 w-4 text-emerald-600 shrink-0 mt-0.5" />
                              <div className="min-w-0">
                                <h5 className="font-black text-xs uppercase tracking-wide text-emerald-950">
                                  EXPEDIENTE ACTUALMENTE OBSERVADO
                                </h5>
                                <p className="text-[11px] text-emerald-800 leading-snug mt-0.5">
                                  ¿El asesor o área subsanó la observación? Puedes marcar que se levantó para reingresarlo al flujo legal.
                                </p>
                              </div>
                            </div>
                            <button
                              ...
                              className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs rounded-full shadow-xs transition-all cursor-pointer shrink-0 text-center self-start sm:self-center"
                            >
                              {liftingRecordId === currentRec.id ? "Cerrar" : "Levantar Observación"}
                            </button>
                          </div>

                          {liftingRecordId === currentRec.id && (
                            <div className="pt-2 border-t border-emerald-200/60 space-y-2 animate-fadeIn">
                              <textarea
                                value={liftComment}
                                onChange={(e) => setLiftComment(e.target.value)}
                                placeholder="Comentario opcional: ej. Se subsanaron las observaciones del asesor y expediente está expedito para emisión..."
                                rows={2}
                                className="w-full text-xs p-2.5 bg-white border border-emerald-300 rounded-xl outline-none focus:ring-1 focus:ring-emerald-500 text-slate-800"
                              />
                              <button
                                type="button"
                                disabled={isLiftingSubmitting}
                                onClick={() => handleConfirmLiftObservation(currentRec)}
                                className="w-full py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs uppercase rounded-xl shadow-xs transition-all cursor-pointer flex items-center justify-center gap-1.5"
                              >
                                <CheckCircle2 className="h-4 w-4" />
