...
export default function JefeLegalPanel({ 
  ...
  const [reopeningTargetRecord, setReopeningTargetRecord] = useState<OperationRecord | null>(null);
  ...
  // PDF Export Modal & Collapsible Records List state
  const [reportModalOpen, setReportModalOpen] = useState(false);
  const [isRecordsListExpanded, setIsRecordsListExpanded] = useState(true);
  ...
  const [pendingActionType, setPendingActionType] = useState<"emit" | "cierre" | "desistido" | "reopen" | null>(null);
  ...
  const getRecordEntryTime = (r: OperationRecord): number => {
    const t = safeGetTime(r.reingresadoAt || r.solicitudAt || r.solicitud || r.createdAt);
  ...
  const isOperationCompleted = (r: OperationRecord): boolean => {
    const s = (r.status || "").toLowerCase();
    return s.includes("cierre") || s.includes("desistid") || s.includes("rechazad") || s.includes("finaliz") || s.includes("entregad");
  };

  const sortedEmitRecords = [...filteredEmitRecords].sort((a, b) => {
    // Operations completed/closed always appear at the very end in Jefe Legal profile (Image 2)
    const aClosed = isOperationCompleted(a);
    const bClosed = isOperationCompleted(b);
    if (aClosed && !bClosed) return 1;
    if (!aClosed && bClosed) return -1;

    // Prioritize re-entered / reopened operations at the top of active operations
    const aReentered = Boolean(a.reingresadoAt);
    const bReentered = Boolean(b.reingresadoAt);
    if (aReentered && !bReentered) return -1;
    if (!aReentered && bReentered) return 1;
  ...
  const dynamicTipoKpis = useMemo(() => {
    const defaultPalette = [
      { border: "border-blue-200", text: "text-blue-700", bg: "bg-blue-100", num: "text-blue-800" },
      { border: "border-amber-200", text: "text-amber-700", bg: "bg-amber-100", num: "text-amber-800" },
      { border: "border-purple-200", text: "text-purple-700", bg: "bg-purple-100", num: "text-purple-800" },
      { border: "border-emerald-200", text: "text-emerald-700", bg: "bg-emerald-100", num: "text-emerald-800" },
      { border: "border-rose-200", text: "text-rose-700", bg: "bg-rose-100", num: "text-rose-800" },
      { border: "border-cyan-200", text: "text-cyan-700", bg: "bg-cyan-100", num: "text-cyan-800" },
      { border: "border-violet-200", text: "text-violet-700", bg: "bg-violet-100", num: "text-violet-800" },
    ];

    const allTypesSet = new Set<string>();
    (tiposOperacion || []).forEach(t => { if (t?.trim()) allTypesSet.add(t.trim().toUpperCase()); });
    activeRecordsForKpi.forEach(r => { if (r.tipo?.trim()) allTypesSet.add(r.tipo.trim().toUpperCase()); });

    const sortedTypes = Array.from(allTypesSet).sort((a, b) => {
      // Keep EMISION, MODIFICACION, ADENDA first if present
    ...
    return sortedTypes.map((tipo, idx) => {
      const pal = defaultPalette[idx % defaultPalette.length];
      const count = activeRecordsForKpi.filter(r => (r.tipo || "").trim().toUpperCase() === tipo).length;
      return {
        tipo,
  ...
  }, [tiposOperacion, activeRecordsForKpi]);
  ...
  const handleSelectEmitRecord = (r: OperationRecord) => {
    ...
    // Al momento de realizar las acciones, los check deben salir sin marcar y el cuadro de comentario en blanco en todos los perfiles
    setEmitSelectedObservations([]);
    setEmitComment("");

    if (isOtherAreasObservation(r)) {
  ...
  // Helper to get the canonical legal observed status (strictly excluding "otras áreas")
  const legalObservedStatus = useMemo(() => {
    return statusesList.find(s => 
      (s.toLowerCase().includes("observad") || s.toLowerCase().includes("rechazad")) &&
      !s.toLowerCase().includes("otras")
    ) || "Observado / Rechazado";
  }, [statusesList]);

  // Helper to get the canonical other areas observed status
  const otherAreasObservedStatus = useMemo(() => {
    return statusesList.find(s => 
      s.toLowerCase().includes("otras áreas") || s.toLowerCase().includes("otras areas")
  ...
  const handleToggleEmitObservation = (obs: string) => {
    setEmitSelectedObservations(prev => {
      ...
      // Auto-suggest status and affectsAdvisor when adding or toggling an observation
      if (!isChecked && next.length > 0) {
        // Any observation from the Legal / Asesor checklist strictly affects advisor in time
        const hasLegal = next.some(o => 
          STANDARD_OBSERVATIONS.includes(o as any) ||
          (!o.toLowerCase().includes("otras áreas") && !o.toLowerCase().includes("otras areas"))
        ) || emitObsCategoryTab === "legal";

        if (hasLegal) {
          setEmitAffectsAdvisor(true);
          setSelectedStatus(legalObservedStatus);
        } else {
          setEmitAffectsAdvisor(false);
          setSelectedStatus(otherAreasObservedStatus);
        }
      }
      return next;
    });
  ...
  const handleOpenEmitSummaryModal = (e?: React.FormEvent) => {
    ...
    const isLegalObs = emitSelectedObservations.length > 0 && (
      emitAffectsAdvisor ||
      emitObsCategoryTab === "legal" ||
      emitSelectedObservations.some(o => STANDARD_OBSERVATIONS.includes(o as any))
    );

    const emissionStatusToSubmit = isLegalObs
      ? legalObservedStatus
      : (emitSelectedObservations.length > 0 && emitObsCategoryTab === "otras_areas" ? otherAreasObservedStatus : selectedStatus);

    const affectsAdvisorToSubmit = isLegalObs ? true : (emitObsCategoryTab === "otras_areas" ? false : emitAffectsAdvisor);

    setSummaryModalData({
      projectName: activeRec?.proyecto || "Expediente",
      unit: unitStr,
      advisor: emitAsesor || activeRec?.asesor || "Sin asignar",
      emissionStatus: emissionStatusToSubmit,
      ...
      affectsAdvisor: affectsAdvisorToSubmit,
  ...
  const executeEmitOperation = () => {
    ...
    const isLegalObs = emitSelectedObservations.length > 0 && (
      emitAffectsAdvisor ||
      emitObsCategoryTab === "legal" ||
      emitSelectedObservations.some(obs => STANDARD_OBSERVATIONS.includes(obs as any))
    );

    const isOther = !isLegalObs && (
      !emitAffectsAdvisor || 
      emitObsCategoryTab === "otras_areas" ||
      emitSelectedObservations.some(obs => 
    ...
    const finalAffectsAdvisor = isLegalObs ? true : (isOther ? false : emitAffectsAdvisor);
    ...
    if (emitSelectedObservations.length > 0) {
      const prefix = isOther ? `[Observación - Otras Áreas]` : `[Observación]`;
    ...
    const finalStatus = isLegalObs
      ? legalObservedStatus
      : (isOther
          ? otherAreasObservedStatus
          : selectedStatus);
    ...
    onUpdateRecord(selectedEmitRecordId, {
      ...
      affectsAdvisor: finalAffectsAdvisor,
  ...
  const handleConfirmSummaryAction = () => {
    ...
    } else if (pendingActionType === "reopen") {
      const targetRec = reopeningTargetRecord || (reopeningRecordId ? records.find(r => r.id === reopeningRecordId) : null) || (selectedHistoryRecordId ? records.find(r => r.id === selectedHistoryRecordId) : null);
      if (targetRec) {
        executeReopenWithAction(targetRec);
      }
    }
  ...
  const assistantsList = useMemo(() => {
    const isInvalidAsst = (n?: string) => {
      if (!n) return true;
      const clean = n.trim().toLowerCase();
      return (
        clean === "asistente legal" ||
        clean === "asistente" ||
        clean === "asistente_legal" ||
        clean === "sin asignar" ||
        clean === "jefe legal" ||
        clean === "jefe_legal" ||
        clean === "administrador" ||
    ...
    const fromUsers = (settings.users || [])
      .filter(u => u.role === "Asistente Legal")
      .map(u => u.username)
      .filter(u => !isInvalidAsst(u));

    const baseNames = ["Franco Odar", "Daniel Hurtado", "Marilyn Saona", "Sthief Vidalon"];
    const combined = [...fromUsers];
    for (const name of baseNames) {
      if (!combined.some(n => n.toLowerCase() === name.toLowerCase()) && !isInvalidAsst(name)) {
        combined.push(name);
      }
    }
    return combined;
  ...
  // Operations that have received attention by legal assistant and are observed or have observations in history or were reopened
  const attendedObservedOperations = useMemo(() => {
    return records.filter(r => {
      ...
      const hasReopened = Boolean(r.reingresadoAt || (Array.isArray(r.history) && r.history.some(h => (h.comentario || "").toLowerCase().includes("reapertura"))));
      return isObs || hasReasons || hasObsText || (hasLegalAttention && normStatus.includes("revis")) || hasReopened;
    });
  }, [records]);
  ...
  // Trigger Action Summary Warning Modal before Reopening
  const handleOpenReopenSummaryModal = (rec: OperationRecord) => {
    if (!reopenNewStatus) {
      alert("Por favor selecciona un nuevo estado legal para la reapertura.");
      return;
    }
    const finalActionType = newActionType === "Otro" ? (customActionType.trim() || "Otro") : newActionType;
    const finalComment = newActionComment.trim() || "Reapertura de trámite y reactivación del flujo operativo por Jefe Legal.";
    const targetAssistant = assignedAssistant.trim() || rec.derivadoA || "Sin Asignar";

    setReopeningRecordId(rec.id);
    setReopeningTargetRecord(rec);
    setSummaryModalData({
      projectName: rec.proyecto,
      unit: formatProjectAndUnits(rec),
      advisor: rec.asesor || "Sin Asesor",
      emissionStatus: reopenNewStatus,
      actionTitle: "ADVERTENCIA: REAPERTURA DE EXPEDIENTE",
      isReopening: true,
      previousStatus: rec.status,
      actionType: finalActionType,
      assignedAssistant: targetAssistant,
      comment: finalComment
    });
    setPendingActionType("reopen");
    setSummaryModalOpen(true);
  };

  // Execution: Reopen closed operation and assign new legal status + action history log
  const executeReopenWithAction = async (rec: OperationRecord) => {
    if (!reopenNewStatus) return;
    setIsReopeningSubmitting(true);
    try {
      const nowIso = new Date().toISOString();
      const finalActionType = newActionType === "Otro" ? (customActionType.trim() || "Otro") : newActionType;
      const finalComment = newActionComment.trim() || "Reapertura de expediente y reactivación de flujo";
      const targetAssistant = assignedAssistant.trim() || rec.derivadoA || "";
      const chosenStatus = reopenNewStatus;

      const historyComment = `[Reapertura de Operación - ${finalActionType}] Estado reactivado de "${rec.status}" a "${chosenStatus}". ${targetAssistant ? `Asignado a ${targetAssistant}. ` : ""}Detalle: ${finalComment}`.trim();
      ...
      await onUpdateRecord(rec.id, {
        ...
        reingresadoAt: nowIso,
        derivadoA: targetAssistant,
        emittedAt: chosenStatus === "Emitido" ? nowIso : null,
        emision: chosenStatus === "Emitido" ? formatDateTimeFull(new Date()) : "",
        comentario: historyComment,
        observacion: chosenStatus.toLowerCase().includes("observad") ? rec.observacion : "",
        observationReasons: chosenStatus.toLowerCase().includes("observad") ? rec.observationReasons : [],
        updatedByUser: currentUser?.username || "Jefe Legal",
      ...
      setReopeningTargetRecord(null);
      setToastMessage(`Operación ${formatProjectAndUnits(rec)} reabierta exitosamente con estado "${chosenStatus}". El tiempo de atención legal está corriendo para el asistente.`);
      setTimeout(() => setToastMessage(null), 4000);
    } catch (e: any) {
  ...
  // Handler: Reopen closed operation directly (fallback)
  const handleConfirmReopen = async (rec: OperationRecord) => {
    handleOpenReopenSummaryModal(rec);
  };
  ...
  return (
    <div className="w-full max-w-6xl mx-auto space-y-6" id="jefe-legal-panel-container">
      ...
      {/* Dashboard Greeting */}
      <section className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-blue-50/20 p-4 rounded-2xl border border-blue-100/50">
        <div>
        ...
        <div className="flex items-center gap-3 self-end sm:self-auto">
          {/* PDF Report Export Button (Imagen 7) */}
          <button
            type="button"
            onClick={() => setReportModalOpen(true)}
            className="px-3.5 py-2 bg-rose-600 hover:bg-rose-700 text-white font-extrabold rounded-xl shadow-xs border border-rose-500 flex items-center gap-2 transition-all cursor-pointer text-xs shrink-0"
            title="Generar y exportar informe PDF oficial por rango de fechas (según plantilla 01/09 al 11/09)"
          >
      ...
      {/* KPI Summary Strip for Jefe Legal: Dynamic breakdown by all registered operation types */}
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500 font-medium">
                Desglose dinámico por tipos registrados y configurados por Administración
              </p>
            </div>
          </div>

          <div className="flex flex-wrap items-center gap-2">
            {/* Dynamically render badges for all operation types configured or registered */}
            {dynamicTipoKpis.map((dt) => (
              <div 
                key={dt.tipo} 
                className={`bg-white px-3 py-1.5 rounded-xl border ${dt.borderColor} shadow-2xs flex items-center gap-2`}
              >
                <span className={`text-[10px] font-extrabold ${dt.textColor} uppercase`}>{dt.label}:</span>
                <span className={`${dt.bgColor} ${dt.numColor} text-xs px-2 py-0.5 rounded-lg font-mono font-black`}>
                  {dt.count}
                </span>
              </div>
            ))}
            ...
            <div className="bg-slate-900 text-white px-3 py-1.5 rounded-xl shadow-xs flex items-center gap-2">
              <span className="text-[10px] font-extrabold text-slate-300 uppercase">Total Operaciones:</span>
              <span className="bg-brand-primary text-white text-xs px-2 py-0.5 rounded-lg font-mono font-black">
                      ...
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 pb-3 border-b border-slate-100">
                        ...
                        <div className="flex flex-col items-start md:items-end gap-1.5 shrink-0">
                          {(() => {
                            ...
                            const isObs = breakdown.currentHolder === "ADVISOR" || breakdown.currentHolder === "OTHER_AREAS";
                            ...
                            return (
                              <div className="flex flex-col items-start md:items-end gap-1 shrink-0">
                                ...
                                <div
                                  ...
                                  title="Tiempo de Respuesta Legal"
                                >
                                  <Clock className="h-3 w-3 shrink-0" />
                                  <span>T. R. Legal: {breakdown.legalFormatted}</span>
                                </div>

                                {/* T. R. Asesor */}
                                {(breakdown.advisorMinutes > 0 || breakdown.currentHolder === "ADVISOR") && (
                                  <div
                                    className={`flex items-center gap-1.5 text-[10px] font-bold px-2.5 py-0.5 rounded-full border ${
                                      breakdown.currentHolder === "ADVISOR"
                                        ? "bg-rose-50 text-rose-700 border-rose-200 animate-pulse"
                                        : "bg-amber-50 text-amber-700 border-amber-200"
                                    }`}
                                    title="Tiempo de Respuesta Asesor"
                                  >
                                    <Clock className="h-3 w-3 shrink-0" />
                                    <span>T. R. Asesor: {breakdown.advisorFormatted}</span>
                                  </div>
                                )}

                                {/* T. Otras Areas */}
                                {(breakdown.otherAreasMinutes > 0 || breakdown.currentHolder === "OTHER_AREAS") && (
                                  <div
                                    className="flex items-center gap-1.5 text-[10px] font-bold px-2.5 py-0.5 rounded-full border bg-blue-50 text-blue-700 border-blue-200"
                                    title="Tiempo Otras Áreas"
                                  >
                                    <Clock className="h-3 w-3 shrink-0 text-blue-600" />
                                    <span>T. Otras Areas: {breakdown.otherAreasFormatted}</span>
                                  </div>
                                )}
                              </div>
                            );
                  ...
                  sortedEmitRecords.map((r, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* Time Breakdown: T. R. Legal, T. R. Asesor, T. Otras Areas matching Image 5 */}
                        {(() => {
                          const breakdown = calculateOperationTimeBreakdown(r, settings.workingSchedule);
                          const isObserved = (r.status || "").toLowerCase().includes("observad") || breakdown.currentHolder === "ADVISOR" || breakdown.currentHolder === "OTHER_AREAS";

                          return (
                            <div className="pt-2 mt-2 border-t border-slate-100 flex flex-col gap-1 text-[10px]">
                              <div className="flex items-center justify-between gap-1">
                                <span className="text-slate-400 font-medium">T. R. Legal:</span>
                                <span className={`font-bold px-2 py-0.5 rounded-full border ${
                                  isObserved ? "bg-slate-50 text-slate-700 border-slate-200" :
                                  breakdown.legalHours < 4 ? "bg-emerald-50 text-emerald-700 border-emerald-200" :
                                  breakdown.legalHours < 8 ? "bg-amber-50 text-amber-700 border-amber-200" :
                                  "bg-rose-50 text-rose-700 border-rose-200"
                                }`}>
                                  {breakdown.legalFormatted}
                                </span>
                              </div>

                              {(breakdown.advisorMinutes > 0 || breakdown.currentHolder === "ADVISOR") && (
                                <div className="flex items-center justify-between gap-1">
                                  <span className="text-slate-400 font-medium">T. R. Asesor:</span>
                                  <span className={`font-bold px-2 py-0.5 rounded-full border ${
                                    breakdown.currentHolder === "ADVISOR" ? "bg-rose-50 text-rose-700 border-rose-200 animate-pulse" : "bg-amber-50 text-amber-700 border-amber-200"
                                  }`}>
                                    {breakdown.advisorFormatted}
                                  </span>
                                </div>
                              )}
                              ...
                              {(breakdown.otherAreasMinutes > 0 || breakdown.currentHolder === "OTHER_AREAS") && (
                                <div className="flex items-center justify-between gap-1">
                                  <span className="text-slate-400 font-medium">T. Otras Areas:</span>
                                  <span className="font-bold px-2 py-0.5 rounded-full border bg-blue-50 text-blue-700 border-blue-200">
                                    {breakdown.otherAreasFormatted}
                                  </span>
                                </div>
                              )}
                              ...
                              <div className="pt-2 mt-1 border-t border-slate-100 flex flex-col gap-1.5">
                                <div className="flex items-center gap-2">
                                  <button
                                    ...
                                    className="flex-1 py-1.5 px-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-extrabold text-[11px] rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1"
                                  ...
                                  <button
                                    ...
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      handleSelectEmitRecord(r);
                                    }}
                                    className={`flex-1 py-1.5 px-2 font-black text-[11px] rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1 shadow-xs ${
                                      isSelected ? "bg-blue-800 text-white ring-2 ring-blue-400" : "bg-brand-primary hover:bg-blue-800 text-white"
                                    }`}
                                  >
                                ...
                                {/* Additional button in Jefe Legal to lift observation directly (Image 4 style) */}
                                {isObserved && (
                                  <div>
                                    {liftingRecordId === r.id ? (
                                      <div className="p-2.5 bg-emerald-50 border border-emerald-300 rounded-xl space-y-2 mt-1 animate-fadeIn">
                                        <span className="text-[10px] font-black text-emerald-950 uppercase block">
                                          Confirmar Levantamiento de Observación:
                                        </span>
                                        <textarea
                                          value={liftComment}
                                          onChange={(e) => setLiftComment(e.target.value)}
                                          placeholder="Comentario sobre subsanación..."
                                          rows={2}
                                          className="w-full text-xs p-2 bg-white border border-emerald-300 rounded-lg outline-none focus:ring-1 focus:ring-emerald-500 text-slate-800"
                                        />
                                        <div className="flex gap-1.5">
                                          <button
                                            type="button"
                                            disabled={isLiftingSubmitting}
                                            onClick={(e) => {
                                              e.stopPropagation();
                                              handleConfirmLiftObservation(r);
                                            }}
                                            className="flex-1 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-[10px] uppercase rounded-lg cursor-pointer"
                                          >
                                            {isLiftingSubmitting ? "Levantando..." : "Confirmar"}
                                          </button>
                                          <button
                                            type="button"
                                            onClick={(e) => {
                                            ...
                                            className="px-2.5 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold text-[10px] rounded-lg cursor-pointer"
                                          >
                                            Cancelar
                                          </button>
                                        </div>
                                      ...
                                      <button
                                        ...
                                        onClick={(e) => {
                                          e.stopPropagation();
                                          setLiftingRecordId(r.id);
                                          setLiftComment("Se subsanó la observación y expediente queda expedito para emisión.");
                                        }}
                                        className="w-full py-2 px-3 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-[11px] rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1.5 shadow-xs uppercase tracking-wide"
                                      >
                                        <CheckCircle2 className="h-4 w-4 shrink-0" />
                                        <span>SE LEVANTÓ LA OBSERVACIÓN</span>
                                      </button>
                                    )}
                                  </div>
                                )}
            ...
            <div className="lg:col-span-7">
              {selectedEmitRecordId ? (
                (() => {
                  ...
                  const isCurrentObserved = currentRec && ((currentRec.status || "").toLowerCase().includes("observad") || (currentRec.status || "").toLowerCase().includes("rechazad"));
                  return (
                    <div className="bg-white p-6 rounded-3xl border border-slate-150 shadow-sm space-y-5">
                      ...
                      <div className="p-4 bg-gradient-to-r from-blue-50 to-slate-50 border border-blue-100 rounded-2xl space-y-2">
                        <div className="flex justify-between items-start gap-3">
                          ...
                          {/* Top-right: Ver Historial button + ID badge */}
                          <div className="flex items-center gap-2 shrink-0">
                            <button
                              type="button"
                              onClick={() => currentRec && setHistoryModalRecord(currentRec)}
                              className="px-3 py-2 bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 font-extrabold text-xs rounded-2xl shadow-xs transition-all cursor-pointer flex items-center gap-1.5"
                              title="Ver historial de esta operación"
                            >
                      ...
                      {/* Subsanación de Observación: Solo botón según Imagen 1 */}
                      {isCurrentObserved && currentRec && (
                        <div className="space-y-2">
                          <button
                            type="button"
                            onClick={() => {
                              if (liftingRecordId === currentRec.id) {
                                setLiftingRecordId(null);
                              } else {
                                setLiftingRecordId(currentRec.id);
                                setLiftComment("Se subsanó la observación y expediente queda expedito para emisión.");
                              }
                            }}
                            className="w-full py-2.5 px-4 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs uppercase tracking-wider rounded-xl shadow-xs transition-all cursor-pointer flex items-center justify-center gap-2"
                          >
                            <CheckCircle2 className="h-4 w-4 shrink-0" />
                            <span>{liftingRecordId === currentRec.id ? "Cancelar Subsanación" : "Levantar Observación"}</span>
                          </button>

                          {liftingRecordId === currentRec.id && (
                            <div className="p-3 bg-emerald-50/90 border border-emerald-300 rounded-xl space-y-2 animate-fadeIn">
                              <textarea
                                value={liftComment}
                                onChange={(e) => setLiftComment(e.target.value)}
                                placeholder="Comentario opcional: ej. Se subsanaron las observaciones del asesor y expediente está expedito para emisión..."
                                rows={2}
                                className="w-full text-xs p-2.5 bg-white border border-emerald-300 rounded-lg outline-none focus:ring-1 focus:ring-emerald-500 text-slate-800"
                              />
                              <button
                                type="button"
                                disabled={isLiftingSubmitting}
                                onClick={() => handleConfirmLiftObservation(currentRec)}
                                className="w-full py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs uppercase rounded-lg shadow-xs transition-all cursor-pointer flex items-center justify-center gap-1.5"
                              >
                                <CheckCircle2 className="h-4 w-4" />
                                <span>{isLiftingSubmitting ? "Levantando Observación..." : "Confirmar y Levantar Observación"}</span>
                              </button>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-1.5 bg-slate-100 p-1 rounded-xl">
                          <button
                            ...
                            onClick={() => {
                              setEmitObsCategoryTab("legal");
                              setEmitAffectsAdvisor(true);
                              if (selectedStatus.toLowerCase().includes("otras")) {
                                setSelectedStatus(legalObservedStatus);
                              }
                            }}
                          ...
                          <button
                            ...
                            onClick={() => {
                              setEmitObsCategoryTab("otras_areas");
                              setEmitAffectsAdvisor(false);
                              if (emitSelectedObservations.length > 0 && !selectedStatus.toLowerCase().includes("otras")) {
                                setSelectedStatus(otherAreasObservedStatus);
                              }
                            }}
                            className={`py-1.5 px-2 rounded-lg text-xs font-black transition-all flex items-center justify-center gap-1.5 cursor-pointer ${
      ...
      {jefeSubTab === "actions_history" && (
        <div className="space-y-4 animate-fadeIn">
          ...
          {/* Header Controls for Records List: Dropdown selection + Hide List toggle (Imagen 6) */}
          <div className="p-3 bg-slate-50 border border-slate-200 rounded-2xl space-y-2.5">
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                <Layers className="h-4 w-4 text-brand-primary" />
                <h4 className="text-xs font-black text-slate-800 uppercase tracking-wide">
                  Listado de Expedientes ({filteredAllRecords.length})
                </h4>
              </div>

              {/* Botón para ocultar o mostrar la lista (Imagen 6) */}
              <button
                type="button"
                onClick={() => setIsRecordsListExpanded(!isRecordsListExpanded)}
                className="py-1 px-3 bg-white hover:bg-slate-100 text-slate-700 border border-slate-200 rounded-xl text-[11px] font-bold transition-all cursor-pointer flex items-center gap-1.5 shadow-2xs"
                title={isRecordsListExpanded ? "Ocultar la lista de expedientes" : "Mostrar la lista de expedientes"}
              >
                {isRecordsListExpanded ? (
                  <>
                  ...
                  <>
                    ...
                    <span>Mostrar Lista ({filteredAllRecords.length})</span>
                  </>
                )}
              </button>
            </div>

            {/* Lista Desplegable para seleccionar expediente directamente (Imagen 6) */}
            <div className="flex flex-col gap-1 pt-1.5 border-t border-slate-200/60">
              <label className="text-[10px] font-bold text-slate-500 uppercase flex items-center gap-1">
                ...
                Seleccionar directamente desde la lista desplegable:
              </label>
              <select
                value={selectedHistoryRecordId || ""}
                onChange={(e) => {
                  const val = e.target.value;
                  setSelectedHistoryRecordId(val || null);
                  if (val) {
                    const rec = records.find(r => r.id === val);
                    if (rec) {
                      setNewActionComment("");
                      setAssignedAssistant(rec.derivadoA || "");
                      setReopenNewStatus("Modificado");
                      setReopenComment("");
                    }
                  }
                }}
                className="w-full h-9 bg-white border border-blue-200 rounded-xl px-2.5 text-xs font-semibold text-slate-800 outline-none focus:ring-1 focus:ring-brand-primary cursor-pointer shadow-2xs"
              >
                <option value="">-- Elige un expediente de la lista desplegable ({filteredAllRecords.length}) --</option>
                {filteredAllRecords.map((r) => {
                  const refNum = `EXP-${r.id.substr(4, 3).toUpperCase()}`;
                  return (
                    <option key={r.id} value={r.id}>
                      [{refNum}] {formatProjectAndUnits(r)} - Asesor: {r.asesor || "Sin Asesor"} ({r.status || "Pendiente"})
                    </option>
                  );
                })}
              </select>
          ...
          {/* Records List (Ocultable según selección del usuario - Imagen 6) */}
          {isRecordsListExpanded && (
            <div className="space-y-2.5 max-h-96 overflow-y-auto pr-1 animate-fadeIn">
              {filteredAllRecords.map((r) => {
          ...
          {selectedHistoryRecordId && (() => {
            ...
            return (
              <div className="bg-white border border-blue-100 rounded-2xl p-4 shadow-md space-y-4 animate-slideIn">
                ...
                {/* Reopen Closed Operation banner or Standard Action Log */}
                {(selectedRec.status === "Cierre Completo" || selectedRec.status === "Desistido") ? (
                  /* WORKFLOW DE REAPERTURA DE OPERACIÓN (IMÁGENES 1, 2, 3, 4, 5) - COLOR NARANJA */
                  <div className="p-4 sm:p-5 bg-gradient-to-b from-amber-50/90 to-orange-50/50 border-2 border-amber-300 rounded-3xl space-y-4 shadow-sm animate-fadeIn">
                    <div className="flex justify-between items-center text-xs pb-2 border-b border-amber-200/70">
                      ...
                      <button
                        ...
                        className="py-1.5 px-3 bg-white hover:bg-amber-100/60 text-amber-900 font-bold text-[11px] rounded-xl transition-all border border-amber-300 cursor-pointer flex items-center gap-1.5 shadow-2xs"
                      >
                    ...
                    {/* Selector de Nuevo Estado Legal */}
                    <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                        {["Modificado", "En Revisión", "Emitido", "Observado / Rechazado", "Pendiente", "Pendiente de Firma"].map((s) => (
                          <button
                            ...
                            className={`py-2 px-2 rounded-xl font-black text-xs border text-center transition-all cursor-pointer active:scale-95 flex items-center justify-center gap-1 ${
                              reopenNewStatus === s
                                ? "bg-amber-600 text-white border-amber-600 shadow-md ring-2 ring-amber-400"
                                : "bg-white text-slate-700 border-amber-200 hover:bg-amber-100/50"
                            }`}
                          >
                            {reopenNewStatus === s && <Check className="h-3 w-3 inline" />}
                            <span>{s}</span>
                          </button>
                        ))}
                      </div>
                    ...
                    {/* Controles de Acción y Reasignación unificados */}
                    <div className="space-y-3 pt-1 border-t border-amber-200/70">
                      <div>
                        <label className="text-[10px] font-bold text-amber-900 block uppercase mb-1">
                          Tipo de Acción *
                        </label>
                        <select
                          value={newActionType}
                          onChange={(e) => setNewActionType(e.target.value)}
                          className="w-full h-9 bg-white border border-amber-300 rounded-xl px-2.5 text-xs font-semibold text-slate-800 outline-none focus:ring-2 focus:ring-amber-500"
                        >
                          ...
                          <option value="Re-Asignación de Asistente">Derivación / Re-Asignación</option>
                      ...
                      {newActionType === "Otro" && (
                        <div className="animate-slideIn">
                          ...
                          <input
                            ...
                            className="w-full h-9 bg-white border border-amber-300 rounded-xl px-2.5 text-xs outline-none focus:ring-2 focus:ring-amber-500"
                          />
                        </div>
                      )}
                      ...
                      <div>
                        <label className="text-[10px] font-bold text-amber-900 block uppercase mb-1">
                          Re-Asignar Asistente Legal a Cargo (Opcional):
                        </label>
                        <select
                          ...
                          className="w-full h-9 bg-white border border-amber-300 rounded-xl px-2.5 text-xs font-semibold text-slate-800 outline-none focus:ring-2 focus:ring-amber-500"
                        >
                          <option value="">-- Mantener asignación actual ({selectedRec.derivadoA || "Ninguno"}) --</option>
                          {assistantsList.map(a => (
                      ...
                      <div>
                        <label className="text-[10px] font-bold text-amber-900 block uppercase mb-1">
                          Detalle o Comentario de la Acción *
                        </label>
                        <textarea
                          value={newActionComment}
                          onChange={(e) => setNewActionComment(e.target.value)}
                          placeholder="Indica el motivo de la reapertura y las instrucciones para el expediente..."
                          rows={3}
                          className="w-full bg-white border border-amber-300 rounded-xl p-2.5 text-xs text-slate-800 outline-none focus:ring-2 focus:ring-amber-500 placeholder:text-slate-400 resize-none"
                        />
                      </div>

                      {/* Botón Imagen 5: CONFIRMAR REAPERTURA, CAMBIAR ESTADO Y ASIGNACIÓN */}
                      <button
                        type="button"
                        disabled={isReopeningSubmitting || !reopenNewStatus}
                        onClick={() => handleOpenReopenSummaryModal(selectedRec)}
                        className={`w-full py-3 text-white font-black text-xs uppercase tracking-wider rounded-xl transition-all shadow-md active:scale-95 cursor-pointer flex items-center justify-center gap-2 ${
                          isReopeningSubmitting || !reopenNewStatus
                            ? "bg-slate-400 cursor-not-allowed opacity-70"
                            : "bg-amber-600 hover:bg-amber-700 active:bg-amber-800 ring-2 ring-amber-500/40"
                        }`}
                      >
                        <RotateCcw className="h-4 w-4" />
                        <span>{isReopeningSubmitting ? "Procesando Reapertura..." : "CONFIRMA REAPERTURA, CAMBIAR ESTADO Y ASIGNACION"}</span>
                      </button>
                    </div>
                  </div>
                ) : (
                  /* FLUJO NORMAL PARA EXPEDIENTES ACTIVOS */
                  <>
      ...
      {/* Official Legal Report PDF Export Modal (Imagen 7) */}
      <LegalReportModal
