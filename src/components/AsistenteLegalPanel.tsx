...
export default function AsistenteLegalPanel({
    ...
    setSelectedObservations(prev => {
      ...
      if (!isChecked && next.length > 0) {
        ...
        if (obsCfg?.affectsAdvisor !== undefined) {
          setAffectsAdvisor(obsCfg.affectsAdvisor);
        } else if (obsCategoryTab === "otras_areas") {
          setAffectsAdvisor(false);
        }
  ...
  const executeSave = () => {
    ...
    const isOther = !affectsAdvisor || selectedObservations.some(obs => 
    ...
    ) || obsCategoryTab === "otras_areas" || statusVal.toLowerCase().includes("otras áreas") || statusVal.toLowerCase().includes("otras areas");
    ...
    const finalStatus = isOther && !statusVal.toLowerCase().includes("otras") && (statusVal.toLowerCase().includes("observad") || statusVal.toLowerCase().includes("rechazad"))
  ...
  const handleConfirmLiftObservation = (rec: OperationRecord) => {
    ...
    try {
      ...
      const newHistoryEntry: StatusHistoryEntry = {
        ...
        user: currentUser?.username || "Asistente Legal",
        affectsAdvisor: false,
        derivadoA: rec.derivadoA || currentUser?.username
      };
