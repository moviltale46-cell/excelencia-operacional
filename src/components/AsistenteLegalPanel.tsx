...
import { safeGetTime, normalizeText, formatUnitDisplay, formatProjectAndUnits, isOtherAreasObservation, checkDuplicateUnitOperation } from "../utils/dateUtils";
...
export default function AsistenteLegalPanel({
  ...
  const handleSelectRecord = (r: OperationRecord) => {
    ...
    // Check if the record already has standard or other areas observations in its comment
    const foundObs: string[] = [];
    const allReasons = getActiveObservationReasons(settings);
    if (r.comentario) {
      allReasons.forEach(obs => {
    ...
    if (r.affectsAdvisor !== undefined) {
      setAffectsAdvisor(r.affectsAdvisor);
  ...
  const executeCloseProcess = (finalStatus: "Cierre Completo" | "Desistido") => {
    ...
    const commentText = commentVal.trim()
      ? (commentVal.startsWith("[") ? commentVal.trim() : `[${finalStatus}] ${commentVal.trim()}`)
      : (finalStatus === "Cierre Completo"
          ? `[Cierre Completo] Expediente finalizado y cerrado con éxito.`
          : `[Desistido] Expediente marcado como desistido.`);
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-start gap-2">
                        ...
                        {/* Waiting Time badge: If observed, legal timer stops and advisor response timer starts! (If other areas, advisor timer is STOPPED in blue) */}
                        {(() => {
                            ...
                            if (obsEvent.isOtherAreas) {
                              return (
                                <div className="flex flex-col items-end gap-1">
                                  ...
                                  <div className="flex items-center gap-1 text-[10px] font-black px-2.5 py-0.5 rounded-full border bg-blue-50 text-blue-700 border-blue-200 shadow-xs" title="Observación de otras áreas: No afecta al asesor y su temporizador se encuentra detenido">
                                    <Clock className="h-3 w-3 text-blue-600" />
                                    <span>⏱️ Otras Áreas • Tiempo Asesor Detenido</span>
                                  </div>
                                </div>
                              );
                            }
                          ...
                          const legalStartTime = item.reingresadoAt || item.solicitudAt || item.solicitud || item.createdAt;
                          return (
                            <div className="flex flex-col items-end gap-1">
                              {item.reingresadoAt && (
                                <span className="text-[9px] font-extrabold px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200">
                                  Observación Levantada • Reingresado
                                </span>
                              )}
                              <div className={`flex items-center gap-1 text-[10px] font-bold px-2.5 py-0.5 rounded-full border ${getWaitingColorClass(legalStartTime)}`} title="Tiempo transcurrido en horario hábil laboral (Lunes a Viernes 9:00am a 6:00pm)">
                                <Clock className="h-3 w-3" />
                                <span>Espera Legal: {getWaitingTimeStr(legalStartTime)}</span>
                              </div>
                            </div>
                          );
                        })()}
