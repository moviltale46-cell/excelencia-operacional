...
import { 
  ...
  ObservationCategory,
  getActiveObservationReasons,
  getActiveObservationReasonConfigs,
  StatusHistoryEntry
} from "../types";
...
import { 
  ...
  Building2, Info, Check, RotateCcw
} from "lucide-react";
import SearchableSelect from "./SearchableSelect";
import StatusHistoryModal from "./StatusHistoryModal";
import { safeGetTime, normalizeText, formatUnitDisplay, formatProjectAndUnits, isOtherAreasObservation, checkDuplicateUnitOperation, formatDateTimeFull } from "../utils/dateUtils";
...
export default function AsistenteLegalPanel({
  ...
  const [obsCategoryTab, setObsCategoryTab] = useState<ObservationCategory>("legal");
  const [affectsAdvisor, setAffectsAdvisor] = useState<boolean>(true);
  ...
  const statusesList = useMemo(() => {
    const base = settings.statuses && settings.statuses.length > 0
      ? [...settings.statuses]
      : ["Pendiente de Firma", "En Revisión Técnica", "Aprobado para Emisión", "Observado / Rechazado", "Desistido", "Cierre Completo"];
    if (!base.some(s => s.toLowerCase().includes("otras áreas") || s.toLowerCase().includes("otras areas"))) {
      base.splice(base.length - 2, 0, "Observado (Otras Áreas)");
    }
    return base;
  }, [settings.statuses]);

  const advisorOptions = useMemo(() => {
    const base = settings.asesores && settings.asesores.length > 0
      ? settings.asesores
      : [
  ...
  const isAssignedToMe = (r: OperationRecord) => {
    ...
    const isDerived = Boolean(r.derivadoA && (
      normalizeText(r.derivadoA) === normalizeText(currentUser.username) ||
      normalizeText(r.derivadoA).includes(normalizeText(currentUser.username)) ||
      normalizeText(currentUser.username).includes(normalizeText(r.derivadoA))
    ));
  ...
  const assistantProjectList = useMemo(() => {
    return Array.from(
      new Set([
        ...myAssignedProjects,
  ...
  const sortedPendingItems = useMemo(() => {
    ...
    return uniquePending.sort((a, b) => {
      const aEmitted = (a.emittedAt && !a.reingresadoAt) ? safeGetTime(a.emittedAt) : null;
      const bEmitted = (b.emittedAt && !b.reingresadoAt) ? safeGetTime(b.emittedAt) : null;

      if (!aEmitted && bEmitted) return -1;
      if (aEmitted && !bEmitted) return 1;

      if (!aEmitted && !bEmitted) {
        // Prioritize re-entered operations (observación levantada o reabierta) at the top of the pending queue
        const aReentered = Boolean(a.reingresadoAt);
        const bReentered = Boolean(b.reingresadoAt);
        if (aReentered && !bReentered) return -1;
        if (!aReentered && bReentered) return 1;

        const aTime = safeGetTime(a.reingresadoAt || a.solicitudAt || a.createdAt) || 0;
        const bTime = safeGetTime(b.reingresadoAt || b.solicitudAt || b.createdAt) || 0;
        return aTime - bTime;
      }
  ...
  const handleSelectRecord = (r: OperationRecord) => {
    ...
    // Al momento de realizar las acciones, los check deben salir sin marcar y el cuadro de comentario en blanco
    setSelectedObservations([]);
    setCommentVal("");

    // If it's an "Otras Áreas" observation, default tab to "otras_areas"
    if (isOtherAreasObservation(r)) {
      setObsCategoryTab("otras_areas");
      setAffectsAdvisor(false);
    } else {
      setObsCategoryTab("legal");
      setAffectsAdvisor(true);
    }
  ...
  const handleToggleObservation = (obs: string) => {
    setSelectedObservations(prev => {
      ...
      if (!isChecked && next.length > 0) {
        ...
        ) || obsCategoryTab === "legal";

        if (hasLegal) {
          setAffectsAdvisor(true);
          setStatusVal(legalObservedStatus);
        } else {
          setAffectsAdvisor(false);
          setStatusVal(otherAreasObservedStatus);
        }
      }
      return next;
    });
  ...
  const handleOpenEmitSummaryModal = () => {
    ...
    // Validation: Check duplicate units in the same project
    if (activeRecord?.proyecto) {
      const dupCheck = checkDuplicateUnitOperation(records, {
        id: selectedRecordId,
        proyecto: activeRecord.proyecto,
        dpto: dptoVal,
        estac: estacVal,
        dep: depVal,
        tipo: activeRecord.tipo
      });
      if (dupCheck.isDuplicate) {
        alert(dupCheck.errorMessage);
        return;
      }
    }
    ...
    const isLegalObs = selectedObservations.length > 0 && (
      affectsAdvisor ||
      obsCategoryTab === "legal" ||
      selectedObservations.some(o => STANDARD_OBSERVATIONS.includes(o as any))
    );

    const emissionStatusToSubmit = isLegalObs
      ? legalObservedStatus
      : (selectedObservations.length > 0 && obsCategoryTab === "otras_areas" ? otherAreasObservedStatus : statusVal);

    const affectsAdvisorToSubmit = isLegalObs ? true : (obsCategoryTab === "otras_areas" ? false : affectsAdvisor);
  ...
  const handleOpenCloseSummaryModal = (finalStatus: "Cierre Completo" | "Desistido") => {
    ...
    // Validation: Check duplicate units if Cierre Completo
    if (finalStatus !== "Desistido" && activeRecord?.proyecto) {
      const dupCheck = checkDuplicateUnitOperation(records, {
        id: selectedRecordId,
        proyecto: activeRecord.proyecto,
        dpto: dptoVal,
  ...
  const executeSave = () => {
    ...
    const isLegalObs = selectedObservations.length > 0 && (
      affectsAdvisor ||
      obsCategoryTab === "legal" ||
      selectedObservations.some(obs => STANDARD_OBSERVATIONS.includes(obs as any))
    );

    const isOther = !isLegalObs && (
      !affectsAdvisor || 
      obsCategoryTab === "otras_areas" ||
      selectedObservations.some(obs => 
    ...
    const finalAffectsAdvisor = isLegalObs ? true : (isOther ? false : affectsAdvisor);
    ...
    if (selectedObservations.length > 0) {
      ...
      const obsPrefix = `${prefix} Motivos: ${selectedObservations.join(" | ")}`;
    ...
    onUpdateRecord(selectedRecordId, {
      ...
      tipoObservacion: isOther ? "Observaciones de otras áreas" : (selectedObservations.length > 0 ? "Asesor" : undefined),
      affectsAdvisor: finalAffectsAdvisor,
      observationReasons: selectedObservations,
      dpto: dptoVal,
      estac: estacVal,
      dep: depVal,
      asesor: asesorVal,
  ...
  // Handler: Lift observation by Legal Assistant (Detiene tiempo asesor/otras áreas y reanuda temporizador legal acumulativo)
  const handleConfirmLiftObservation = (rec: OperationRecord) => {
    if (!rec) return;
    setIsLiftingSubmitting(true);
    try {
      const now = new Date();
      const nowIso = now.toISOString();
      const comment = liftComment.trim()
        ? `[Observación Levantada] ${liftComment.trim()}`
        : `[Observación Levantada] Se subsanó la observación por el Asesor / Área correspondiente. Se reanuda el tiempo de atención Legal.`;

      const asstName = currentUser?.username || rec.derivadoA || "Franco Odar";
      const newHistoryEntry: StatusHistoryEntry = {
        status: "Pendiente",
        comentario: comment,
        timestamp: formatDateTimeFull(now),
        user: asstName,
        affectsAdvisor: false,
        derivadoA: rec.derivadoA || asstName
      };
      ...
      onUpdateRecord(rec.id, {
        ...
        updatedByUser: asstName,
        history: [...(rec.history || []), newHistoryEntry]
      });

      setLiftingRecordId(null);
      setLiftComment("");
      setToastMessage(`Observación levantada para ${formatProjectAndUnits(rec)}. Se reanudó el tiempo de atención legal.`);
  ...
  const getActionSummary = (record: OperationRecord) => {
    ...
    const parsed = historyList.map(h => {
      ...
      return { type, comment, user: h.user || record.updatedByUser || currentUser?.username || record.derivadoA || "Franco Odar", timestamp: h.timestamp, status: h.status };
    });
  ...
  const handleRegisterAction = (recordId: string) => {
    ...
    const isOther = finalActionType === "Observaciones de otras áreas" || finalActionType.toLowerCase().includes("otras áreas");
    
    onUpdateRecord(recordId, {
      comentario: `[${finalActionType}] ${newActionComment.trim()}`,
      ...(isOther ? {
        tipoObservacion: "Observaciones de otras áreas",
        status: "Observado (Otras Áreas)"
      } : {}),
      updatedByUser: currentUser?.username || "Franco Odar"
    });
  ...
  userProcessedEmissions.forEach(r => {
    ...
    if (breakdown.legalMinutes > 0) {
      totalUserMinutes += breakdown.legalMinutes;
  ...
  const getObservationEvent = (item: OperationRecord) => {
    ...
    const obsHistory = item.history?.slice().reverse().find(h => 
      ...
      (h.comentario && (h.comentario.toLowerCase().includes("[observación") || isOtherAreasObservation(h)))
    );
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 px-4" id="asistente-legal-panel-container">
      ...
      {/* Stats Counter Section (Reorganized spaces: Title top, Icon and Number below matching Image 6) */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3.5" id="asistente-stats-container">
        {/* Card 1: Solicitudes Totales */}
        <div className="bg-white p-4 rounded-2xl border border-slate-150 shadow-xs flex flex-col justify-between min-h-[96px]">
        ...
        {/* Card 2: Operaciones Emitidas */}
        <div className="bg-white p-4 rounded-2xl border border-slate-150 shadow-xs flex flex-col justify-between min-h-[96px]">
          <span className="text-[10px] font-black text-slate-500 block uppercase tracking-wider">
            OPERACIONES EMITIDAS
          </span>
          <div className="flex items-center gap-3 mt-2">
            <div className="w-10 h-10 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center shrink-0 border border-emerald-100/70">
        ...
        {/* Card 3: Observadas Pendientes */}
        <div className="bg-white p-4 rounded-2xl border border-slate-150 shadow-xs flex flex-col justify-between min-h-[96px]">
          <span className="text-[10px] font-black text-rose-600 block uppercase tracking-wider">
            OBSERVADAS PENDIENTES
          </span>
          <div className="flex items-center gap-3 mt-2">
            <div className="w-10 h-10 rounded-xl bg-rose-50 text-rose-600 flex items-center justify-center shrink-0 border border-rose-100/70">
          ...
          <div className="flex items-center gap-3 mt-2">
            ...
            <div className="leading-tight">
              <div className="flex items-baseline gap-1">
                <span className="text-2xl font-black text-slate-900 tracking-tight">
                  {userAvgResponseMinutes !== null ? userAvgResponseMinutes : 0}
                </span>
            ...
            <section className="bg-white border border-slate-150 rounded-3xl p-5 shadow-xs space-y-4">
              ...
              {selectedRecordId ? (
                (() => {
                  ...
                  const isCurrentObserved = currentSelected && (
                    (currentSelected.status || "").toLowerCase().includes("observad") ||
                    (currentSelected.status || "").toLowerCase().includes("rechazad") ||
                    isOtherAreasObservation(currentSelected)
                  );
                  const isLiftingCurrent = currentSelected && liftingRecordId === currentSelected.id;
                  ...
                  return (
                    <div className="space-y-3">
                      ...
                      {isCurrentObserved && currentSelected && (
                        <div className="space-y-2">
                          <button
                            ...
                            onClick={() => {
                              if (isLiftingCurrent) {
                                ...
                                setLiftingRecordId(currentSelected.id);
                                setLiftComment("");
                              }
                            }}
                          ...
                          >
                            ...
                            <span>{isLiftingCurrent ? "Cancelar Subsanación" : "Levantar Observación"}</span>
                          </button>
                          ...
                          {isLiftingCurrent && (
                            <div className="p-3 bg-emerald-50/90 border border-emerald-300 rounded-xl space-y-2 animate-fadeIn">
                              <textarea
                                ...
                                placeholder="Comentario opcional: ej. Se subsanaron las observaciones del asesor y expediente está conforme..."
                                rows={2}
                              ...
                              <button
                                ...
                                onClick={() => handleConfirmLiftObservation(currentSelected)}
                              ...
                              >
                                ...
                                <span>{isLiftingSubmitting ? "Reingresando..." : "Confirmar que se Levantó la Observación"}</span>
                              </button>
                            </div>
                          )}
                        </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3 text-xs">
                  ...
                  {/* Reactive duplicate detection warning in Asistente Legal Panel */}
                  {(() => {
                    const activeRecord = records.find(r => r.id === selectedRecordId);
                    const dupCheck = activeRecord?.proyecto && (dptoVal.trim() || estacVal.trim() || depVal.trim())
                    ...
                    if (dupCheck?.isDuplicate) {
                      return (
                        <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl text-amber-800 text-[11px] flex items-start gap-2 animate-fadeIn mt-2">
                  ...
                  <div className="grid grid-cols-2 gap-1.5 bg-slate-100 p-1 rounded-xl">
                    <button
                      ...
                      onClick={() => {
                        setObsCategoryTab("legal");
                        setAffectsAdvisor(true);
                        if (statusVal.toLowerCase().includes("otras")) {
                          setStatusVal(legalObservedStatus);
                        }
                      }}
                    ...
                    >
                      <span className={`h-2 w-2 rounded-full ${obsCategoryTab === "legal" ? "bg-rose-500" : "bg-slate-300"}`} />
                      <span>Legal / Asesor</span>
                      {(() => {
                        const legalCount = selectedObservations.filter(o => !STANDARD_OTHER_AREAS_OBSERVATIONS.includes(o as any) && !o.toLowerCase().includes("otras áreas") && !o.toLowerCase().includes("otras areas")).length;
                        return legalCount > 0 ? (
                    ...
                    <button
                      ...
                      onClick={() => {
                        setObsCategoryTab("otras_areas");
                        setAffectsAdvisor(false);
                        if (selectedObservations.length > 0 && !statusVal.toLowerCase().includes("otras")) {
                          setStatusVal(otherAreasObservedStatus);
                        }
                      }}
                      className={`py-1.5 px-2 rounded-lg text-xs font-black transition-all flex items-center justify-center gap-1.5 cursor-pointer ${
                    ...
                    >
                      <span className={`h-2 w-2 rounded-full ${obsCategoryTab === "otras_areas" ? "bg-white" : "bg-blue-500"}`} />
                      <span>Otras Áreas</span>
                      {(() => {
                        const otherCount = selectedObservations.filter(o => STANDARD_OTHER_AREAS_OBSERVATIONS.includes(o as any) || o.toLowerCase().includes("otras áreas") || o.toLowerCase().includes("otras areas")).length;
                        return otherCount > 0 ? (
                          <span className={`text-[10px] px-1.5 py-0.2 rounded-full font-bold ${obsCategoryTab === "otras_areas" ? "bg-white text-blue-700" : "bg-blue-200 text-blue-900"}`}>
                            {otherCount}
                          </span>
                        ) : null;
                      })()}
                                ...
                                <div
                                  ...
                                  className={`flex items-start gap-2.5 p-2.5 rounded-xl border transition-all select-none ${
                                    !selectedRecordId 
                                      ...
                                      : isChecked
                                      ? "bg-blue-50/95 border-blue-400 text-blue-950 shadow-xs ring-1 ring-blue-300 cursor-pointer"
                                      : "bg-blue-50/20 hover:bg-blue-50/60 border-blue-150 text-slate-700 cursor-pointer"
                                  }`}
                                >
                                  <input
                                    type="checkbox"
            ...
            <div className="space-y-4">
              {sortedPendingItems.length === 0 ? (
                ...
                sortedPendingItems.map((item) => {
                  ...
                  const hasBeenActedOn = !!item.emittedAt && !item.reingresadoAt;
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-start gap-2">
                        ...
                        {/* Waiting Time badge: T. R. Legal, T. R. Asesor, T. Otras Areas matching Image 5 */}
                        {(() => {
                          const breakdown = calculateOperationTimeBreakdown(item, settings.workingSchedule);
                          const isObs = breakdown.currentHolder === "ADVISOR" || breakdown.currentHolder === "OTHER_AREAS";

                          return (
                            <div className="flex flex-col items-end gap-1 shrink-0">
                              {/* Running Reopened Timer for Assistant */}
                              {(breakdown.wasReopened || Boolean(item.reingresadoAt)) && (
                                <div
                                  className="flex items-center gap-1 text-[10px] font-black px-2.5 py-0.5 rounded-full border bg-amber-50 text-amber-900 border-amber-300 animate-pulse"
                                  title="Expediente reabierto por Jefe Legal - El tiempo legal está corriendo activamente"
                                >
                                  <RotateCcw className="h-3 w-3 text-amber-600 shrink-0" />
                                  <span>Tiempo Reabierto: {breakdown.reopenedFormatted || "En curso"}</span>
                                </div>
                              )}
                              ...
                              <div
                                className={`flex items-center gap-1 text-[10px] font-bold px-2.5 py-0.5 rounded-full border ${
                                  isObs
                                    ...
                                    : breakdown.legalHours < 4
                                    ? "text-emerald-700 bg-emerald-50 border-emerald-200"
                                    : breakdown.legalHours < 8
                              ...
                              {(breakdown.otherAreasMinutes > 0 || breakdown.currentHolder === "OTHER_AREAS") && (
                                <div
                                  className="flex items-center gap-1 text-[10px] font-bold px-2.5 py-0.5 rounded-full border bg-blue-50 text-blue-700 border-blue-200"
                      ...
                      <div className="flex justify-between items-center text-[10px] border-t border-slate-100 pt-2.5">
                        ...
                        <div className="flex items-center gap-1.5">
                          {(() => {
                            ...
                            if (isOther) {
                              return (
                                <span className="px-2.5 py-0.5 rounded text-[9px] font-black uppercase tracking-wide border bg-blue-50 text-blue-700 border-blue-200 shadow-xs flex items-center gap-1">
                                  <span className="h-1.5 w-1.5 rounded-full bg-blue-600"></span>
                                  {item.status.includes("Otras Áreas") ? item.status : "Observado (Otras Áreas)"}
                                </span>
                              );
                            }
                            return (
                      ...
                      <div className="space-y-2 pt-1">
                        ...
                        {/* Inline Button to mark Observation Lifted if currently observed */}
                        {(() => {
                          const obsEvent = getObservationEvent(item);
                          const isObs = Boolean(obsEvent) || (item.status || "").toLowerCase().includes("observad") || (item.status || "").toLowerCase().includes("rechazad");
                          if (!isObs) return null;

                          const isLiftingThis = liftingRecordId === item.id;
                              ...
                              {isLiftingThis ? (
                                <div className="p-3 bg-emerald-50/90 border border-emerald-300 rounded-xl space-y-2 animate-fadeIn">
                                  ...
                                  <textarea
                                    value={liftComment}
                                    onChange={(e) => setLiftComment(e.target.value)}
                                    placeholder="Comentario opcional: ej. Se subsanó por el asesor..."
                                    rows={2}
                                    className="w-full text-xs p-2 bg-white border border-emerald-200 rounded-lg outline-none focus:ring-1 focus:ring-emerald-500 text-slate-800"
                                  />
                                  <button
                                    type="button"
                                    disabled={isLiftingSubmitting}
                                    onClick={() => handleConfirmLiftObservation(item)}
                                    className="w-full py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-[10px] uppercase rounded-lg shadow-xs flex items-center justify-center gap-1 cursor-pointer active:scale-95"
                                  >
                                ...
                                <button
                                  type="button"
                                  onClick={() => {
                                    setLiftingRecordId(item.id);
                                    setLiftComment("");
                                  }}
                                  className="w-full py-1.5 px-2 bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold rounded-lg text-[10px] uppercase flex items-center justify-center gap-1.5 transition-all shadow-xs cursor-pointer active:scale-95"
                                  title="Marcar que se levantó la observación para detener el tiempo del asesor y reanudar el tiempo de atención legal"
                                >
                                  <CheckCircle2 className="h-3.5 w-3.5" />
                      ...
                      >
                        <div className="flex justify-between items-center gap-1">
                          <div className="flex items-center gap-1.5 min-w-0 flex-wrap">
                            {(() => {
                              const isOther = isOtherAreasObservation({ comentario: act.comment, status: act.type } as any) ||
                                act.type.toLowerCase().includes("otras áreas") ||
                                act.type.toLowerCase().includes("otras areas");
                              return (
                                <span className={`border text-[9px] font-black px-2 py-0.5 rounded uppercase truncate ${
                                  isOther 
                  ...
                  <div className="space-y-2 text-xs">
                    <div>
                      ...
                      >
                        ...
                        <option value="Observaciones de otras áreas">Observaciones de otras áreas (Color Azul)</option>
                    ...
                    {newActionType === "Observaciones de otras áreas" && (
                      <div className="p-2 bg-blue-50 border border-blue-200 rounded-xl text-blue-900 text-[11px] flex items-center gap-2 animate-fadeIn">
                        <Info className="h-3.5 w-3.5 text-blue-600 shrink-0" />
                        <span>Esta acción registrará una observación de otras áreas: <strong>No afecta el KPI del asesor y detiene su temporizador</strong>.</span>
                      </div>
                    )}
