...
import { safeParseDate, safeGetTime, formatDateTimeFull, formatDateOnly, formatProjectAndUnits, isInitialHistoryEntry } from "../utils/dateUtils";
...
export default function DailyObservationsControlChart({
  ...
  // 6. Selected Assistant & Line Toggles for "asistentes" dimension
  const [selectedAssistantFilter, setSelectedAssistantFilter] = useState<string>("ALL");
  const [showAsstTotal, setShowAsstTotal] = useState(true);
  const [showAsstCierres, setShowAsstCierres] = useState(true);
  const [showAsstObs, setShowAsstObs] = useState(true);
  const [showAsstRev, setShowAsstRev] = useState(true);
  ...
  const [drilldownTab, setDrilldownTab] = useState<"acciones" | "expedientes">("acciones");
  ...
  // Jefe Legal users list
  const jefeUsers = useMemo(() => (settings?.users || []).filter(u => u.role === "Jefe Legal"), [settings]);
  const jefeLabels = useMemo(() => {
    const list = jefeUsers.map(u => u.username?.trim()).filter(Boolean) as string[];
    if (!list.some(n => n.toLowerCase().includes("marilyn"))) {
      list.push("Marilyn Saona");
    }
    return list;
  }, [jefeUsers]);

  // Helper to filter out placeholder or role names as actual assistant identities (Image 11)
  const isInvalidAsst = (name?: string) => {
    if (!name) return true;
  ...
  // Helper to resolve legal actor (Assistant or Jefe Legal)
  const getLegalActorName = (r: OperationRecord): string => {
    const jefeNames = jefeUsers.map(u => (u.username || "").toLowerCase());
    const updatedBy = (r.updatedByUser || "").toLowerCase();
    const isJefeAction = jefeNames.includes(updatedBy) || (r.derivadoA && jefeNames.includes(r.derivadoA.toLowerCase()));
    
    if (r.derivadoA?.trim() && !isInvalidAsst(r.derivadoA)) {
      if (jefeNames.includes(r.derivadoA.toLowerCase())) {
        return `${r.derivadoA.trim()} (Jefe Legal)`;
      }
      return r.derivadoA.trim();
    }
    if (isJefeAction) {
      return `${r.updatedByUser || "Jefe Legal"} (Jefe Legal)`;
    }
    return "Sin Asignar";
  };
  ...
  const uniqueAssistants = useMemo(() => {
    const fromUsers = (settings?.users || [])
      .filter(u => (u.role === "Asistente Legal" || u.role === "Jefe Legal") && u.active && !isInvalidAsst(u.username))
      .map(u => u.role === "Jefe Legal" ? `${u.username} (Jefe Legal)` : u.username);
    const fromRecs = records.map(r => getLegalActorName(r)).filter(a => a !== "Sin Asignar" && !isInvalidAsst(a));
    
    // Also include anyone who took action in history
    const fromHistory: string[] = [];
    records.forEach(r => {
      r.history?.forEach(h => {
        const hu = (h.user || "").trim();
        if (!hu || isInvalidAsst(hu)) return;
        const huLower = hu.toLowerCase();
        const isJefe = jefeLabels.some(l => {
          const lClean = l.replace(" (Jefe Legal)", "").toLowerCase().trim();
          return huLower.includes(lClean) || lClean.includes(huLower);
        }) || huLower.includes("marilyn") || huLower.includes("jefe legal");

        if (isJefe) {
          fromHistory.push("Marilyn Saona (Jefe Legal)");
        } else {
          fromHistory.push(hu);
        }
      });
    ...
    // Ensure Marilyn Saona (Jefe Legal) is always in the list
    if (!fromUsers.some(u => u.includes("Marilyn") || u.includes("Jefe Legal")) && !fromHistory.some(u => u.includes("Marilyn"))) {
      fromHistory.push("Marilyn Saona (Jefe Legal)");
    }

    return Array.from(new Set([...fromUsers, ...fromRecs, ...fromHistory])).filter(a => !isInvalidAsst(a)).sort();
  }, [records, settings, jefeLabels]);
  ...
  const dailySeries = useMemo(() => {
    const daysArr: Array<{
      ...
      byAssistantActions: Record<string, {
        total: number;
        cierres: number;
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      for (const r of recsOnDay) {
        ...
        // Assistant tracking (including Jefe Legal as legal actor)
        const asst = getLegalActorName(r);
        if (!byAssistant[asst]) {
      ...
      // ACCIONES REALIZADAS SOBRE LAS OPERACIONES EN ESTE DÍA (Requerimiento de Imagen 2: excluye ingreso de la operación)
      const byAssistantActions: Record<string, {
        total: number;
        cierres: number;
        observaciones: number;
        levantamientos: number;
      ...
      // Initialize all known assistants and Jefe Legal
      uniqueAssistants.forEach(aName => {
        byAssistantActions[aName] = {
          total: 0,
          cierres: 0,
          observaciones: 0,
          levantamientos: 0,
      ...
      // Scan all records for actions performed on this day
      for (const r of records) {
        if (selectedProjectFilter !== "ALL" && r.proyecto !== selectedProjectFilter) continue;
        if (selectedTeamFilter !== "ALL" && r.team !== selectedTeamFilter) continue;

        let hasHistoryActionOnDay = false;

        if (Array.isArray(r.history) && r.history.length > 0) {
          r.history.forEach((h, idx) => {
            // Exclude initial registration of operation
            ...
            if (!d) return;
            if (d.getFullYear() !== selectedYear || d.getMonth() !== selectedMonth || d.getDate() !== day) return;

            hasHistoryActionOnDay = true;
            ...
            const actorKey = isJefe ? "Marilyn Saona (Jefe Legal)" : (hu || "Sin Asignar");

            if (!byAssistantActions[actorKey]) {
              byAssistantActions[actorKey] = {
                total: 0,
                cierres: 0,
                observaciones: 0,
                levantamientos: 0,
            ...
            const isCierre = st.includes("cierre") || st.includes("aprobado para emisi") || st.includes("emitid");
            const isObs = st.includes("observad") || st.includes("rechazad");
            const isLev = st.includes("levantad") || st.includes("levantamiento") || (h.comentario || "").toLowerCase().includes("levantad");
            const isMod = st.includes("modific") || normalizeTipo(r.tipo) === "MODIFICACION";
            const isAden = normalizeTipo(r.tipo) === "ADENDA";

            byAssistantActions[actorKey].total++;
            if (isCierre) byAssistantActions[actorKey].cierres++;
            else if (isObs) byAssistantActions[actorKey].observaciones++;
            else if (isLev) byAssistantActions[actorKey].levantamientos++;
            else if (isMod) byAssistantActions[actorKey].modificaciones++;
            else if (isAden) byAssistantActions[actorKey].adendas++;

            if (!byAssistantActions[actorKey].records.some(rec => rec.id === r.id)) {
              byAssistantActions[actorKey].records.push(r);
            }

            byAssistantActions[actorKey].actionsList.push({
              id: `${r.id}-act-${idx}`,
              opId: r.id,
              proyecto: formatProjectAndUnits(r),
              dpto: r.dpto || "-",
              asesor: r.asesor || "-",
              team: r.team || "A",
              user: hu || (isJefe ? "Marilyn Saona" : "Asistente"),
              status: h.status || "Acción Realizada",
              timestamp: h.timestamp || "",
              comentario: h.comentario || "",
              assistantHelped: isJefe ? (r.derivadoA || "Franco Odar") : undefined
            });
          });
        }

        // Si no hubo acción en historial, verificar fecha de emisión
        if (!hasHistoryActionOnDay && (r.emittedAt || r.emision)) {
          const d = safeParseDate(r.emittedAt || r.emision);
          if (d && d.getFullYear() === selectedYear && d.getMonth() === selectedMonth && d.getDate() === day) {
            const actorKey = getLegalActorName(r);
            if (!byAssistantActions[actorKey]) {
              byAssistantActions[actorKey] = {
                total: 0,
                cierres: 0,
            ...
            byAssistantActions[actorKey].total++;
            byAssistantActions[actorKey].cierres++;
            if (!byAssistantActions[actorKey].records.some(rec => rec.id === r.id)) {
              byAssistantActions[actorKey].records.push(r);
            }
            byAssistantActions[actorKey].actionsList.push({
              ...
              team: r.team || "A",
              user: actorKey,
              status: r.status || "Cierre Completo",
              timestamp: r.emittedAt || r.emision || "",
              comentario: "Emisión / Cierre registrado"
            });
          }
        }
      }
    ...
    return daysArr;
  }, [records, currentMonthRecords, daysInMonth, selectedMonth, selectedYear, dateBasis, selectedProjectFilter, selectedTeamFilter, uniqueAssistants, jefeLabels]);
  ...
  const weeklySeries = useMemo(() => {
    ...
    for (let w = 1; w <= totalWeeks; w++) {
      ...
      // Aggregate actions across days of the week
      const byAssistantActions: Record<string, {
        total: number;
        cierres: number;
        observaciones: number;
      ...
      for (let day = startDay; day <= endDay; day++) {
        const dayItem = dailySeries[day - 1];
        if (dayItem && dayItem.byAssistantActions) {
          Object.entries(dayItem.byAssistantActions).forEach(([actor, data]: [string, any]) => {
            if (!byAssistantActions[actor]) {
              byAssistantActions[actor] = {
                total: 0,
                cierres: 0,
                observaciones: 0,
                levantamientos: 0,
            ...
            byAssistantActions[actor].total += data.total;
            byAssistantActions[actor].cierres += data.cierres;
            byAssistantActions[actor].observaciones += data.observaciones;
            byAssistantActions[actor].levantamientos += data.levantamientos;
            byAssistantActions[actor].modificaciones += data.modificaciones;
            byAssistantActions[actor].adendas += data.adendas;
            data.records.forEach((r: OperationRecord) => {
              if (!byAssistantActions[actor].records.some(rec => rec.id === r.id)) {
                byAssistantActions[actor].records.push(r);
              }
            });
            byAssistantActions[actor].actionsList.push(...data.actionsList);
          });
        }
      }
    ...
    return weeks;
  }, [currentMonthRecords, dailySeries, daysInMonth, selectedMonth, dateBasis]);
  ...
  // Assistant performance for the month based on ACTIONS PERFORMED & SUPPORTED (Requerimiento de usuario)
  const assistantPerformance = useMemo(() => {
    ...
    uniqueAssistants.forEach(asstName => {
      map[asstName] = {
        name: asstName,
        totalActions: 0,
        cierres: 0,
    ...
    dailySeries.forEach(dayItem => {
      if (dayItem.byAssistantActions) {
        Object.entries(dayItem.byAssistantActions).forEach(([actor, data]: [string, any]) => {
          if (!map[actor]) {
            map[actor] = {
              name: actor,
          ...
          map[actor].totalActions += data.total;
          map[actor].cierres += data.cierres;
          map[actor].observaciones += data.observaciones;
          map[actor].levantamientos += data.levantamientos;
          map[actor].modificaciones += data.modificaciones;
          map[actor].adendas += data.adendas;
          map[actor].total = map[actor].totalActions;
          map[actor].emision = map[actor].cierres;
          map[actor].modificacion = map[actor].modificaciones;
          map[actor].adenda = map[actor].adendas;
        });
      }
    });

    return Object.values(map).sort((a, b) => b.totalActions - a.totalActions);
  }, [uniqueAssistants, dailySeries]);
  ...
  const linesToDraw = useMemo(() => {
    ...
    if (analysisDimension === "asistentes") {
      if (selectedAssistantFilter === "ALL") {
        // Show all assistants + Jefe Legal lines comparing their daily actions performed/supported
        const topAssistants = assistantPerformance.length > 0 ? assistantPerformance : uniqueAssistants.map(name => ({ name }));
        const palette = ["#7c3aed", "#2563eb", "#059669", "#d97706", "#e11d48", "#0891b2"];
        return topAssistants.map((asst, idx) => {
          const isJefe = asst.name.toLowerCase().includes("jefe legal") || asst.name.toLowerCase().includes("marilyn");
          return {
            key: asst.name,
            label: isJefe ? `@${asst.name} (Acciones Apoyadas)` : `@${asst.name} (Acciones Realizadas)`,
            color: isJefe ? "#7c3aed" : palette[(idx + 1) % palette.length],
            visible: true,
            getValue: (item: any) => item.byAssistantActions?.[asst.name]?.total || 0
          };
        });
      } else {
        // Show selected assistant breakdown of daily actions performed/supported
        const isJefe = selectedAssistantFilter.toLowerCase().includes("jefe legal") || selectedAssistantFilter.toLowerCase().includes("marilyn");
        return [
          {
            key: "asst_total",
            label: isJefe ? "Total Acciones Apoyadas" : `${selectedAssistantFilter} - Total Acciones`,
            color: isJefe ? "#7c3aed" : "#2563eb",
            visible: showAsstTotal,
            getValue: (item: any) => item.byAssistantActions?.[selectedAssistantFilter]?.total || 0
          },
          {
            key: "asst_cierres",
            label: isJefe ? "Cierres y Aprobaciones Apoyadas" : "Cierres y Emisiones Realizadas",
            color: "#059669",
            visible: showAsstCierres,
            getValue: (item: any) => item.byAssistantActions?.[selectedAssistantFilter]?.cierres || 0
          },
          {
            key: "asst_obs",
            label: "Observaciones Formuladas",
            color: "#e11d48",
            visible: showAsstObs,
            getValue: (item: any) => item.byAssistantActions?.[selectedAssistantFilter]?.observaciones || 0
          },
          {
            key: "asst_rev",
            label: "Revisiones / Levantamientos",
            color: "#d97706",
            visible: showAsstRev,
            getValue: (item: any) => 
              (item.byAssistantActions?.[selectedAssistantFilter]?.levantamientos || 0) +
              (item.byAssistantActions?.[selectedAssistantFilter]?.modificaciones || 0) +
              (item.byAssistantActions?.[selectedAssistantFilter]?.adendas || 0)
          }
        ];
      }
    }
    ...
    // Default
    return [];
  }, [analysisDimension, showLineEmision, showLineModificacion, showLineAdenda, showLineTotal, selectedAssistantFilter, showAsstTotal, showAsstCierres, showAsstObs, showAsstRev, assistantPerformance, uniqueAssistants, selectedAdvisorFilter, advisorRanking]);
  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-200/90 shadow-sm p-5 md:p-6 space-y-6" id="operational-kpi-chart-section">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 p-3 bg-slate-50/70 rounded-2xl border border-slate-200/60">
        ...
        {/* If dimension is "asistentes": Dropdown to filter by specific assistant & line toggles */}
        {analysisDimension === "asistentes" && (
          <div className="flex flex-wrap items-center gap-2 text-xs">
            ...
            >
              <option value="ALL">-- Ver Todos Comparados (Acciones por Día) --</option>
            ...
            {selectedAssistantFilter !== "ALL" && (
              <div className="flex flex-wrap items-center gap-2 ml-1">
                {/* Total Acciones Checkbox */}
                <label className="flex items-center gap-1.5 font-black text-purple-700 bg-purple-50 px-2.5 py-1 rounded-xl border border-purple-200 cursor-pointer select-none">
                  <input
                    type="checkbox"
                    checked={showAsstTotal}
                    onChange={(e) => setShowAsstTotal(e.target.checked)}
                    className="rounded accent-purple-600"
                  />
                  <span className="h-2.5 w-2.5 rounded-full bg-purple-600 inline-block"></span>
                  Total Acciones
                ...
                <label className="flex items-center gap-1.5 font-bold text-emerald-800 bg-emerald-50 px-2.5 py-1 rounded-xl border border-emerald-200 cursor-pointer select-none">
                  <input
                    type="checkbox"
                    checked={showAsstCierres}
                    onChange={(e) => setShowAsstCierres(e.target.checked)}
                    className="rounded accent-emerald-600"
                  />
                  <span className="h-2.5 w-2.5 rounded-full bg-emerald-600 inline-block"></span>
                  Cierres / Aprobados
                ...
                {/* Observaciones Checkbox */}
                <label className="flex items-center gap-1.5 font-bold text-rose-800 bg-rose-50 px-2.5 py-1 rounded-xl border border-rose-200 cursor-pointer select-none">
                  <input
                    type="checkbox"
                    checked={showAsstObs}
                    onChange={(e) => setShowAsstObs(e.target.checked)}
                ...
                {/* Revisiones / Levantamientos Checkbox */}
                <label className="flex items-center gap-1.5 font-bold text-amber-800 bg-amber-50 px-2.5 py-1 rounded-xl border border-amber-200 cursor-pointer select-none">
                  <input
                    type="checkbox"
                    checked={showAsstRev}
                    onChange={(e) => setShowAsstRev(e.target.checked)}
                    className="rounded accent-amber-500"
                  />
                  <span className="h-2.5 w-2.5 rounded-full bg-amber-500 inline-block"></span>
                  Revisiones / Levant.
            ...
            <span className="text-[10px] text-emerald-700 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-full font-bold">
              ✓ Cantidad de acciones realizadas (sin ingreso de operación)
            </span>
          </div>
        )}
              ...
              {activeSeries.map((item, idx) => {
                ...
                return (
                  <rect
                    ...
                    onClick={() => {
                      const actionsForPeriod = (analysisDimension === "asistentes" && selectedAssistantFilter !== "ALL")
                        ? (item.byAssistantActions?.[selectedAssistantFilter]?.actionsList || [])
                        : Object.values(item.byAssistantActions || {}).flatMap((a: any) => a.actionsList || []);

                      setSelectedPeriodDetail({
                        label: item.label,
                        dateFormatted: "dateFormatted" in item ? item.dateFormatted : (item as any).rangeStr,
                        records: item.records,
                        actionsList: actionsForPeriod
                      });
                      setDrilldownTab(analysisDimension === "asistentes" && actionsForPeriod.length > 0 ? "acciones" : "expedientes");
                    }}
                  />
                );
              })}
              ...
              >
                ...
                <div className="space-y-1 pt-1 font-mono">
                  ...
                  <div className="pt-1.5 border-t border-slate-800 flex justify-between text-[11px] text-slate-300 font-bold">
                    <span>{analysisDimension === "asistentes" ? "Acciones totales del día:" : "Total expedientes:"}</span>
                    <span className="text-emerald-400 font-black">
                      {analysisDimension === "asistentes"
                        ? Object.values(activeSeries[hoveredIdx].byAssistantActions || {}).reduce((sum: number, a: any) => sum + (a.total || 0), 0)
                        : activeSeries[hoveredIdx].total}
                    </span>
                  </div>
                </div>
      ...
      {/* 7. DRILLDOWN EXPEDIENT & ACTIONS DETAIL PANEL (WHEN CLICKING A DAY/ITEM) */}
        ...
        <div className="p-5 bg-slate-50 border border-slate-200 rounded-3xl space-y-4 animate-fadeIn">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-200">
            <div>
              <h4 className="text-sm font-extrabold text-slate-900 uppercase tracking-wider flex items-center gap-2">
                ...
                Detalle del {selectedPeriodDetail.label} ({selectedPeriodDetail.dateFormatted})
              </h4>
              <p className="text-xs text-slate-500 font-medium">
                {selectedPeriodDetail.actionsList?.length || 0} acciones realizadas | {selectedPeriodDetail.records.length} expedientes asociados
              </p>
            </div>

            <div className="flex items-center gap-2">
              {/* Tab Selector: Acciones vs Expedientes */}
              <div className="flex items-center bg-white p-1 rounded-2xl border border-slate-200 text-xs">
                <button
                  onClick={() => setDrilldownTab("acciones")}
                  className={`px-3 py-1 font-extrabold rounded-xl transition-all cursor-pointer ${
                    drilldownTab === "acciones"
                      ? "bg-purple-600 text-white shadow-xs"
                      : "text-slate-600 hover:text-slate-900"
                  }`}
                >
                  Acciones Realizadas ({selectedPeriodDetail.actionsList?.length || 0})
                </button>
                <button
                  onClick={() => setDrilldownTab("expedientes")}
                  className={`px-3 py-1 font-extrabold rounded-xl transition-all cursor-pointer ${
                    drilldownTab === "expedientes"
                      ? "bg-blue-600 text-white shadow-xs"
                      : "text-slate-600 hover:text-slate-900"
                  }`}
                >
                  Expedientes ({selectedPeriodDetail.records.length})
                </button>
              </div>

              <button
                onClick={() => setSelectedPeriodDetail(null)}
                className="text-xs font-bold text-slate-500 hover:text-slate-800 bg-white border border-slate-200 px-3 py-1.5 rounded-xl cursor-pointer"
              >
                Cerrar ✕
              </button>
            </div>
          ...
          {drilldownTab === "acciones" ? (
            <div className="space-y-2">
              <div className="flex items-center justify-between text-[11px] text-purple-900 bg-purple-50 border border-purple-200 px-3 py-1.5 rounded-xl font-medium">
                <span>⚡ <strong>Acciones operativas del día:</strong> Cierres, revisiones, observaciones y gestiones realizadas sobre las operaciones (excluye el registro inicial).</span>
                <span className="font-mono font-bold text-purple-700">{selectedPeriodDetail.actionsList?.length || 0} acciones</span>
              </div>

              <div className="overflow-x-auto max-h-[340px] rounded-xl border border-slate-200 bg-white">
                <table className="w-full text-left border-collapse text-xs">
                  <thead className="bg-slate-100 text-slate-700 font-black text-[10px] uppercase sticky top-0">
                    <tr>
                      <th className="p-2.5">Operación / Proyecto</th>
                      <th className="p-2.5">Asesor</th>
                      <th className="p-2.5">Actor (Asistente / Legal)</th>
                      <th className="p-2.5">Acción Realizada</th>
                      <th className="p-2.5">Fecha y Hora</th>
                      <th className="p-2.5">Comentario / Detalle</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {!selectedPeriodDetail.actionsList || selectedPeriodDetail.actionsList.length === 0 ? (
                      <tr>
                        <td colSpan={6} className="p-6 text-center text-slate-400 italic">
                          No se registraron acciones adicionales sobre las operaciones en este día.
                        </td>
                      </tr>
                    ) : (
                      selectedPeriodDetail.actionsList.map(act => (
                        <tr key={act.id} className="hover:bg-purple-50/30 transition-colors">
                          <td className="p-2.5">
                            <div className="font-mono text-[9px] text-slate-400">ID: {act.opId}</div>
                            <span className="font-extrabold text-slate-800">{act.proyecto}</span>
                            <span className="text-[10px] text-slate-500 block">Dpto: {act.dpto}</span>
                          </td>
                          <td className="p-2.5">
                            <span className="font-bold text-slate-700 uppercase block">{act.asesor}</span>
                            <span className="text-[9px] text-slate-400 font-semibold">Team {act.team}</span>
                          </td>
                          <td className="p-2.5">
                            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-extrabold ${
                              act.user.toLowerCase().includes("marilyn") || act.user.toLowerCase().includes("jefe")
                            ...
                            {act.assistantHelped && (
                              <span className="text-[9px] text-slate-400 block mt-0.5">
                                Apoyo a: {act.assistantHelped}
                              </span>
                            )}
                          </td>
                          <td className="p-2.5">
                            <span className="font-extrabold text-slate-800 bg-slate-100 px-2 py-0.5 rounded text-[10px] border border-slate-200">
                              {act.status}
                            </span>
                          </td>
                          <td className="p-2.5 font-mono text-xs text-slate-600 font-semibold">
                            {formatDateTimeFull(act.timestamp)}
                          </td>
                          <td className="p-2.5 max-w-xs text-slate-600 font-medium">
                            <p className="line-clamp-2 text-[11px]">{act.comentario || "Sin comentario adicional"}</p>
                          </td>
                        </tr>
                      ))
                    )}
                    ...
                    selectedPeriodDetail.records.map(r => (
                      <tr key={r.id} className="hover:bg-blue-50/40 transition-colors">
                        <td className="p-2.5">
                          ...
                          <span className="font-extrabold text-slate-800">{formatProjectAndUnits(r)}</span>
                        </td>
                        <td className="p-2.5">
