...
export default function DailyObservationsControlChart({
  ...
  const uniqueAssistants = useMemo(() => {
    const fromUsers = (settings?.users || [])
      .filter(u => (u.role === "Asistente Legal" || u.role === "Jefe Legal") && u.active)
      .map(u => u.role === "Jefe Legal" ? `${u.username} (Jefe Legal)` : u.username);
    const fromRecs = records.map(r => getLegalActorName(r)).filter(a => a !== "Sin Asignar");
    ...
    return Array.from(new Set([...fromUsers, ...fromRecs, ...fromHistory])).sort();
