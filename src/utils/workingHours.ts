...
export function calculateOperationTimeBreakdown(
  ...
  // If operation was reopened or reingresado, ensure reingresadoAt is represented in the timeline
  if (record.reingresadoAt) {
    const reenteredDate = safeParseDate(record.reingresadoAt);
    if (reenteredDate) {
      rawEvents.push({
        date: reenteredDate,
        status: record.status || "Modificado",
  ...
  for (let i = 1; i < events.length; i++) {
    ...
    if (isObservationLiftedEvent(nextEvent) || (nextEvent.comment && nextEvent.comment.toLowerCase().includes("reapertura"))) {
      // "Se levantó la observación" or "Reapertura" -> Clock stops for closed/advisor, resumes for Legal
      currentRole = 'LEGAL';
      if (nextEvent.comment && nextEvent.comment.toLowerCase().includes("reapertura")) {
        wasReopened = true;
      }
  ...
  // If record has reingresadoAt or wasReopened and its current status is not closed, it is active in LEGAL role
  ...
  const isCurrentlyClosed = (!hasReentered && !wasReopened) ? (isStatusClosed || currentRole === 'CLOSED') : isStatusClosed;
  ...
  if ((wasReopened || hasReentered) && !isCurrentlyClosed) {
    const reenteredDate = safeParseDate(record.reingresadoAt) || currentTime;
  ...
  return {
    ...
    isCurrentlyActive,
    wasReopened: wasReopened || hasReentered,
    reopenedMinutes,
    reopenedHours,
    reopenedFormatted,
    hadObservations,
