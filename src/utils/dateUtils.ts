...
/**
 * Helper to identify whether a history entry is merely the initial ticket creation/request/ingreso
 * and NOT an actual operational action performed on the operation.
 */
export function isInitialHistoryEntry(
  h: { status?: string; comentario?: string; timestamp?: string; isInitial?: boolean },
  recordSolicitud?: string
): boolean {
  if (!h) return false;
  ...
  if (
    ...
    status.includes("registro inicial") ||
    status.includes("registro de operaci") ||
    status.includes("registro de la operaci")
  ) {
    return true;
  }

  if (status === "pendiente" && (comment.includes("inicial") || comment.includes("sistema") || comment.includes("registrada") || !comment)) {
    return true;
  }

  if (recordSolicitud && h.timestamp && h.timestamp.trim() === recordSolicitud.trim() && (!status || status === "pendiente" || status.includes("registro"))) {
    return true;
  }
...
/**
 * Checks if a status string represents a completed closure ("CIERRE COMPLETO", "Emitido", etc.)
 */
export function isCierreCompletoStatus(status?: string): boolean {
  ...
  return (
    ...
    s.includes("cierre") ||
    s.includes("emitid") ||
    s.includes("aprobado para emisión") ||
    s.includes("aprobado para emision") ||
    s.includes("minuta emitida") ||
...
/**
 * Checks if a status string represents an observed / rejected status
 */
export function isObservadoStatus(status?: string): boolean {
  if (!status) return false;
  const s = status.trim().toLowerCase();
  return s.includes("observad") || s.includes("rechazad");
}
...
export interface ResolvedRecordAction {
  ...
  latestObservationReasons: string[];
  isCierreCompleto: boolean;
  isObservado: boolean;
  isObservadoAsesor: boolean;
  isObservadoOtrasAreas: boolean;
}

/**
 * Evaluates an operation record against a selected date range and inclusion filters.
 * STRICT PRINCIPLE: Always show only the latest status/action on that date ("siempre la ultima accion"),
 * never older history from other dates, and never qualify records purely by background updatedAt touches.
 * Allows multi-selection of status filters: Cierres Completos, Observados (Asesor), and Observados (Otras Áreas).
 */
export function resolveRecordActionInRange(
  r: any,
  startDate?: string,
  endDate?: string,
  includeEmisiones: boolean = true,
  includeStatusChanges: boolean = true,
  onlyCierresCompletos: boolean = false,
  onlyObservados: boolean = false,
  onlyObservadasOtrasAreas: boolean = false
): ResolvedRecordAction {
  const currentStatus = r.status || "Pendiente";
  const isCurrentlyCierre = isCierreCompletoStatus(currentStatus);
  const isCurrentlyObs = isObservadoStatus(currentStatus);
  const isCurrentlyOtherAreas = isOtherAreasObservation(r) || 
                                currentStatus.toLowerCase().includes("otras áreas") || 
                                currentStatus.toLowerCase().includes("otras areas");
  const isCurrentlyObsAsesor = isCurrentlyObs && !isCurrentlyOtherAreas;

  // If no date range is set, evaluate overall current state
  if (!startDate && !endDate) {
    let inRange = true;
    const hasAnyStatusFilter = onlyCierresCompletos || onlyObservados || onlyObservadasOtrasAreas;
    if (hasAnyStatusFilter) {
      inRange = (onlyCierresCompletos && isCurrentlyCierre) ||
                (onlyObservados && isCurrentlyObsAsesor) ||
                (onlyObservadasOtrasAreas && isCurrentlyOtherAreas);
    }
    ...
    return {
      ...
      latestDate: r.solicitud || r.createdAt || "",
      latestComment: r.observacion || r.comentario || "",
      latestObservationReasons: r.observationReasons || [],
      isCierreCompleto: isCurrentlyCierre,
      isObservado: isCurrentlyObs,
      isObservadoAsesor: isCurrentlyObsAsesor,
      isObservadoOtrasAreas: isCurrentlyOtherAreas
    };
  }
  ...
  const checkDateWithin = (dCandidate?: string | Date | null): Date | null => {
    if (!dCandidate) return null;
    const d = safeParseDate(dCandidate);
    if (!d) return null;
    if (startBoundary && d.getTime() < startBoundary.getTime()) return null;
    if (endBoundary && d.getTime() > endBoundary.getTime()) return null;
    return d;
  };

  // 1. Check Emission / Solicitud in range
  const emissionDateObj = checkDateWithin(r.solicitudAt) ||
                          checkDateWithin(r.solicitud) ||
                          checkDateWithin(r.createdAt) ||
                          checkDateWithin(r.emision) ||
                          checkDateWithin(r.emittedAt);
  const isEmission = !!emissionDateObj;

  // 2. Check Status changes in range
  // We inspect actual operational events (history entries, emittedAt, reingresadoAt).
  // IMPORTANT: We do NOT check r.updatedAt because updatedAt is updated on unrelated cache/reads.
  let latestHistoryInRange: any = null;
  let latestHistoryTime = 0;

  if (Array.isArray(r.history) && r.history.length > 0) {
    for (const h of r.history) {
      if (!h || !h.timestamp) continue;
      const d = checkDateWithin(h.timestamp);
      if (d) {
        const time = d.getTime();
        if (time >= latestHistoryTime) {
          latestHistoryTime = time;
          latestHistoryInRange = h;
        }
      }
    }
  }

  const reingresoDate = checkDateWithin(r.reingresadoAt);
  const emittedDate = checkDateWithin(r.emittedAt);

  let isStatusChange = false;
  if (latestHistoryInRange && !isInitialHistoryEntry(latestHistoryInRange, r.solicitud)) {
    isStatusChange = true;
  } else if (reingresoDate) {
    isStatusChange = true;
  } else if (emittedDate && isCierreCompletoStatus(r.status)) {
    isStatusChange = true;
  } else if (isEmission && (isCierreCompletoStatus(r.status) || isObservadoStatus(r.status))) {
    // If the record was requested and also closed/observed on the same date
    isStatusChange = true;
  }

  // Determine inclusion based on checkboxes
  let matchesInclusion = false;
  if (includeEmisiones && includeStatusChanges) {
    matchesInclusion = isEmission || isStatusChange;
  } else if (includeEmisiones) {
    matchesInclusion = isEmission;
  } else if (includeStatusChanges) {
    matchesInclusion = isStatusChange;
  }
  ...
  if (!matchesInclusion) {
    return {
      ...
      isStatusChange,
      latestStatus: currentStatus,
      latestDate: r.solicitud || "",
      latestComment: "",
      latestObservationReasons: [],
      isCierreCompleto: isCurrentlyCierre,
      isObservado: isCurrentlyObs,
      isObservadoAsesor: isCurrentlyObsAsesor,
      isObservadoOtrasAreas: isCurrentlyOtherAreas
  ...
  // Resolve the "Última Acción" on this selected date
  let latestStatus = currentStatus;
  let latestDate = r.solicitud || r.createdAt || "";
  let latestComment = r.observacion || r.comentario || "";
  let latestObservationReasons: string[] = r.observationReasons || [];

  if (latestHistoryInRange) {
    latestStatus = latestHistoryInRange.status || currentStatus;
    latestDate = latestHistoryInRange.timestamp || r.solicitud || "";
    if (latestHistoryInRange.comentario && latestHistoryInRange.comentario.trim()) {
      latestComment = latestHistoryInRange.comentario.trim();
    }
    if (Array.isArray(latestHistoryInRange.observationReasons) && latestHistoryInRange.observationReasons.length > 0) {
      latestObservationReasons = latestHistoryInRange.observationReasons;
    }
  } else {
    latestStatus = currentStatus;
    latestDate = r.solicitud || r.createdAt || "";
  ...
  const isCierre = isCierreCompletoStatus(latestStatus);
  const isObs = isObservadoStatus(latestStatus);
  
  // Check if this latest action corresponds to "Observaciones de otras áreas"
  const isOtherAreasAction = isOtherAreasObservation(latestHistoryInRange || r) ||
                             latestStatus.toLowerCase().includes("otras áreas") ||
                             latestStatus.toLowerCase().includes("otras areas");
  const isObsOtrasAreas = isOtherAreasAction;
  const isObsAsesor = isObs && !isOtherAreasAction;

  // Format status display for other areas if needed
  if (isOtherAreasAction && isObs && !latestStatus.toLowerCase().includes("otras áreas") && !latestStatus.toLowerCase().includes("otras areas")) {
    latestStatus = "OBSERVADO (OTRAS ÁREAS)";
  }

  // Multi-select status filtering:
  // If ANY status filter is active, record must match at least one of the selected filters
  const hasAnyStatusFilter = onlyCierresCompletos || onlyObservados || onlyObservadasOtrasAreas;
  if (hasAnyStatusFilter) {
    const matchesSelectedStatus = 
      (onlyCierresCompletos && isCierre) ||
      (onlyObservados && isObsAsesor) ||
      (onlyObservadasOtrasAreas && isObsOtrasAreas);
    ...
    if (!matchesSelectedStatus) {
      return {
        ...
        latestComment,
        latestObservationReasons,
        isCierreCompleto: isCierre,
        isObservado: isObs,
        isObservadoAsesor: isObsAsesor,
        isObservadoOtrasAreas: isObsOtrasAreas
      };
    }
  }
